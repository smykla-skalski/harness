...
Do not pre-select weak signals because speculative groups create review noise and make the suite look more complete than the code evidence supports.
...
- G3: Runtime config - xDS correctness for metrics/traces/logs clusters and listeners
- G4: End-to-end flow - data reaches collectors
- G5: Edge cases - backendRef acceptance, dangling refs, and protocol mismatch
- G6: Multi-zone - KDS sync and zone runtime behavior
- G7: Backward compatibility - deprecated inline endpoint behavior and migration warnings

**Detected variants and resulting groups:**

| Signal | Variant | Groups | Count |
| --- | --- | --- | --- |
| S3 | Mixed backend usage (OTel + Prometheus) | G8-G9 | 2 |
| S2 | Unified naming mode | G10 | 1 |
| S6+S4 | Pipe mode protocol behavior | G11-G14 | 4 |
| S1 | Universal multi-zone parity | G15-G18 | 4 |
| S3 | Endpoint optionality and schema parity | G19 | 1 |

**Total**: 7 base + 12 variant = 19 groups.

The variant signals came from additional backend types (S3), naming modes (S2), protocol branching for pipe vs network (S6), feature-flagged behavior (S4), and topology-specific Universal parity checks (S1). The base G6 and G7 groups still cover the default multi-zone and backward-compatibility paths.
