...
# and it is why these stay twelve separate invocations.
#
# The caller wraps this in `cargo-local.sh --with-group-lease` so the twelve
# share one build lease instead of twelve, which is what stops them from sizing
