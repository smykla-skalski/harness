...
# and it is why these stay sixteen separate invocations.
#
# The caller wraps this in `cargo-local.sh --with-group-lease` so the sixteen
# share one build lease instead of sixteen, which is what stops them from
