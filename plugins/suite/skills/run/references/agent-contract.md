...
9. When a suite group provides inline manifest YAML, use it verbatim. Do not change names, namespaces, labels, or any fields. If a manifest fails validation or apply, follow the manifest error handling flow in [validation.md](validation.md): move the runner into failure triage, ask the exact canonical user approval prompt gate, and only allow `Fix in suite and this run` to touch the approved suite file plus `amendments.md`. Never silently fix a manifest - the error might indicate a real product bug.
