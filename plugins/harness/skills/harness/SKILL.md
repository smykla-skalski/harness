---
name: harness
description: Use when orchestrating multi-agent sessions, observing agent behavior, managing tasks, or sending signals. First arg is top-level command (session, observe).
argument-hint: <session|observe> <subcommand> [options]
allowed-tools: Agent, Bash, Edit, Glob, Grep, Read, Write
user-invocable: true
---

<!-- justify: CF-tools-usage Workers execute tasks requiring Edit/Write/Glob/Grep; user approval prompt for session picker and confirmations -->
...
<!-- justify: AQ-option-structure user approval prompt used for session picker which presents numbered list dynamically -->
