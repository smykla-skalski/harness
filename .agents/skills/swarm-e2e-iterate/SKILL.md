...
description: Use when running the Harness Monitor swarm full-flow e2e loop in Codex, triaging recordings, invoking the globally installed council skill, or closing open findings in the iteration ledger.
---
...
7. Council review: with `active.md` frozen and non-empty, invoke the globally installed council via `Skill: council` with `Arguments: <mode> @_artifacts/active.md`. Pin `<mode>` to `core-eng`, `core-ux`, `core-mix`, or `debate` based on the Subsystem mix per [council-review.md](references/council-review.md) - never `all` (cost/loop-cadence rule), never bare `core` unless the mix is genuinely unclear. Save the synthesis to `_artifacts/runs/<slug>/council-review.md`. Skip if `active.md` carries zero data rows.
