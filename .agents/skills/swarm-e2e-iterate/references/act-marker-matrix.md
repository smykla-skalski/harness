...
The swarm full-flow lane writes `<act>.ready` and `<act>.ack` markers in the sync directory. Use this table to map each act to the harness side effect, marker payload, and required Monitor surface.
...
| `act5` | Observer injected heuristic codes and ran `session observe`. | `observer_id`, `heuristic_code` (`python_traceback_output` for the assertion) | One `heuristicIssueCard.<code>` per injected code. Missing cards or duplicate labels are findings. |
...
| `act8` | Worker codex submitted `task_review` for review. | `task_review_id`, `worker_codex_id` | `awaitingReviewBadge(task_review_id)` visible inside the inspector. Badge must read AwaitingReview. |
| `act9` | Reviewer claude claimed; duplicate same-runtime claim rejected; reviewer codex claimed; both reviewers approved. | `task_review_id`, `reviewer_runtime` (`claude`) | One of `reviewerClaimBadge(task_review_id, claude)` or `reviewerQuorumIndicator(task_review_id)`. Quorum surface shows two runtimes. |
...
| `act12` | Two extra reviewers joined; arbitration task assigned and started; first request-changes round submitted on point `p1`. | `task_arbitration_id`, `point_id` (`p1`) | One of `partialAgreementChip(p1)`, `reviewPointChip(p1)`, or `roundCounter(task_arbitration_id)`. Round counter pill at value `1`. |
...
- `taskInspectorCard.value` must match the selected task ID.
