...
`harness-daemon remote pair create` produces a short-lived `harness://pair` invitation containing the HTTPS endpoint, one-time code, requested role/scopes, expiry, and the daemon certificate's SPKI SHA-256 pin. Every pairing flow shares that host; the app tells the remote-daemon flow from the relay flow by the payload (a remote invitation carries `server_spki_sha256`), not the host. Links already handed out under the legacy `harness://remote-pair` host still resolve. The phone accepts the same QR, deep-link, and manual-entry flows as local pairing. `URLSessionMobileRemoteDaemonPairingTransport` claims `POST /v1/remote/pair/claim` through a pinning URLSession and binds the claim to a stable client id derived from the phone identity.
