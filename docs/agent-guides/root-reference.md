...
To make a default target lane fast without sharing writable Cargo state, `cargo-local.sh` seeds a previously absent lane from the most recently active idle donor under `target/dev`, using artifact count as a tie-breaker. Lane names carry a format version, and only a donor with the same version is eligible; incrementing `HARNESS_CARGO_LANE_FORMAT_VERSION` therefore forces a clean compatibility boundary after cache-infrastructure changes. It holds the donor's Cargo build lock, clones `debug` with APFS `clonefile` on macOS or mandatory reflinks on Linux, atomically publishes the destination, then lets Cargo validate every fingerprint. The pinned nightly enables Cargo's `checksum-freshness`, so identical files remain fresh across worktrees even though Git gave them different mtimes; build-script inputs still use mtimes until Cargo extends checksum tracking to them. A filesystem without copy-on-write support starts cold instead of performing a multi-gigabyte full copy. Explicit target-directory overrides are never seeded or added to the automatic sccache basedir inventory. `mise run cargo:cache:canary` gives identical source files different mtimes, races two seeders, requires the second isolated lane to report the crate as fresh, deletes the donor, and then rebuilds a branch-local change with copied build-script state; this guards Cargo artifact reuse independently of compiler-cache hits.
