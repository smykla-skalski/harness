...
An empty `SCCACHE_BIN` means no sccache, the way `rustc-cache-wrapper.sh` has always read it, and not "look for one". The difference is not academic: the shell tests pass an empty value for scenarios that have nothing to do with caching, and while it read as "go find one" each of those started a real server inside a sandbox directory the suite then deleted, leaving it to run out its idle timeout while enforcing its own copy of the size limit over the shared cache. The suite now also asserts that it leaves no server listening under its own roots, and stops any it finds.
