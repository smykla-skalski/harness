...
Find every crate that pulls in a given file with `grep -rn 'path = ".*<file>' crates/`. A file reached only through an included `mod.rs` - for example anything under `src/agents/acp/`, reached via `src/agents/acp/mod.rs` - does not show up on a direct grep for itself, so also check whether an ancestor module is included.

Because each include resolves against a different module tree, a change that compiles cleanly in the root crate can still fail, or expose a different symbol set, inside a facade crate that a package-scoped gate never builds. `mise run harness:check:rust` (part of `mise run check`) clippys every crate in the list above and is the run that predicts whether the repository check passes; `cargo clippy -p <owning-crate>` alone does not.
