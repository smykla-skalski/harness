...
`cargo-local.sh` closes that off by starting the one server itself, before cargo runs. It connects to the socket to see whether a server is already answering, and starts one only when nothing is, under a `mkdir` lock beside the socket so that concurrent builds cannot both decide to start. Both halves are chosen to run on either platform: macOS ships no `flock(1)`, which is why the lock is a directory, and `SO_PEERCRED` is Linux only, which is why the probe asks a connect rather than who owns the socket. A timeout or a permission error reads as reachable and starts nothing, because the only action taken on "no server" is starting one. A probe that could not run at all - `/usr/bin/python3` on a macOS without the command line tools is present on `PATH` and exits 1 - falls back to the socket file, so it starts a server only where no socket file exists at all, which is a path no client could reach either. `HARNESS_SCCACHE_PRESTART=0` turns the whole step off. The socket sweep answers the same question the same way and unlinks only a socket a connect proves dead: asking `lsof` which sockets were live and deleting the rest needed `lsof` installed and had to parse its name field, whose format changed between 4.91 and 4.95, so on a modern `lsof` it unlinked the live server's socket on every run and manufactured the very "no server" the next client answered by starting one.
