...
One Cargo command owns a target lane at a time. A second command fails immediately with status 75 and reports the target plus the owner's PID and command instead of waiting behind Cargo's internal build lock. The lock supervisor closes its descriptor before launching children, so an sccache daemon or another background descendant cannot keep a completed command's lane locked.
