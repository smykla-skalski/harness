...
sccache can cache non-incremental Rust dependency compilations, but it is not a cross-worktree cache: its Rust key hashes rustc's working directory and source arguments, and `SCCACHE_BASEDIRS` does not normalize those inputs. It also declines incremental compilations, which includes edited workspace crates here. A same-lane Cargo rerun normally invokes no compiler at all because Cargo's own fingerprints are fresh, while a new worktree changes the sccache key and misses; that is why an unchanged rerun may show zero sccache hits without indicating a broken server. `RUSTC_WRAPPER` is expected to print empty in `mise run cargo:env`: `.cargo/config.toml` owns `scripts/rustc-cache-wrapper.sh`, and an environment override would replace that repository configuration rather than prove caching is enabled.
