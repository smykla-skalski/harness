...
`cargo-local.sh` closes that off by starting the one server itself, before cargo runs. It connects to the socket to see whether a server is already answering, and starts one only when nothing is, under a `mkdir` lock beside the socket so that concurrent builds cannot both decide to start. Both halves are chosen to run on either platform: macOS ships no `flock(1)`, which is why the lock is a directory, and `SO_PEERCRED` is Linux only, which is why the probe asks a connect rather than who owns the socket. A timeout or a permission error reads as reachable and starts nothing, because the only action taken on "no server" is starting one. A probe that could not run at all - `/usr/bin/python3` on a macOS without the command line tools is present on `PATH` and exits 1 - falls back to the socket file, so it starts a server only where no socket file exists at all, which is a path no client could reach either. `HARNESS_SCCACHE_PRESTART=0` turns the whole step off. The socket sweep feeds this: it unlinks socket files that `lsof` does not report as live, and reading only the older `lsof` name format made it unlink the live server's socket on every run, which manufactured the very "no server" the next client answered by starting one.

Which sccache runs is decided by version, not by PATH order alone. `cargo-local.sh` walks every directory on `PATH` and takes the first binary at 0.14 or newer, because `command -v sccache` answers with the first one installed and says nothing about whether it is usable: a distro package in `/usr/bin` ahead of the pinned build - the ordinary shape inside `mise run`, where tool directories land after the system ones - otherwise switched the compiler cache off for the whole repository, and a disabled cache looks exactly like a host with no sccache at all. When something is found but nothing is new enough, the reason is printed rather than left to `CACHE_MODE=none`.
