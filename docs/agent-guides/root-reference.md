...
The suite-lifecycle hooks (`guard-stop`, `context-agent`, `validate-agent`) and the `HARNESS_FEATURE_SUITE_HOOKS` flag that gated them have been removed; only the tool lifecycle remains.
