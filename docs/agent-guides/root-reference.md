...
sccache can cache non-incremental Rust dependency compilations, but it is not a cross-worktree cache: its Rust key hashes rustc's working directory and source arguments, and `SCCACHE_BASEDIRS` does not normalize those inputs. It also declines incremental compilations, which includes edited workspace crates here. A same-lane Cargo rerun normally invokes no compiler at all because Cargo's own fingerprints are fresh, while a new worktree changes the sccache key and misses; that is why an unchanged rerun may show zero sccache hits without indicating a broken server.
...
`mise run clean:sccache` finds the live server by connecting and reading `SO_PEERCRED`, then stops the others. Reach for it when `~/.cache/sccache` sits far below its budget, or to clear orphans left by a build that predates the prestart. It is Linux only, because it reads `/proc` and `SO_PEERCRED`, and the task skips itself elsewhere.

`mise run cargo:cache:status` queries the socket from `cargo-local.sh --print-env`, not sccache's default state, and reports whether that configured server is healthy, cold, unreachable, or accompanied by safely identifiable orphan servers. It also reports request, hit, miss, and non-cacheable counts plus the physical cache path, birth time, and size. `clean:caches` keeps the 100 GiB disk guard because leaked independent servers each enforce their own 30 GiB view while writing one shared cache and can collectively outrun a single-server budget. Before a forced or over-threshold removal it stops the configured server, writes a bounded 100-entry attribution record to the ignored `.cache/diagnostics/sccache-cleanup.jsonl`, and only then removes the cache; a failed stop or audit preserves the cache, while dry-run prints the same decision and writes nothing.
