...
`cargo-local.sh` closes that off by starting the one server itself, before cargo runs. It connects to the socket to see whether a server is already answering, and starts one only when nothing is, under a `mkdir` lock beside the socket so that concurrent builds cannot both decide to start. Both halves are chosen to run on either platform: macOS ships no `flock(1)`, which is why the lock is a directory, and `SO_PEERCRED` is Linux only, which is why the probe asks a connect rather than who owns the socket. Only a definite "nothing is listening" starts a server. A timeout, a permission error, or a probe that could not run at all - `/usr/bin/python3` on a macOS without the command line tools is present on `PATH` and exits 1 - falls back to the socket file and starts nothing, because the only action taken on "no server" is starting one. `HARNESS_SCCACHE_PRESTART=0` turns the whole step off. The socket sweep feeds this: it unlinks socket files that `lsof` does not report as live, and reading only the older `lsof` name format made it unlink the live server's socket on every run, which manufactured the very "no server" the next client answered by starting one.
