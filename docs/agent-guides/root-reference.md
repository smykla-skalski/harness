...
That one directory feeds two test targets, which differ in the library they link rather than in how many tests they hold. `tests/integration.rs` declares the modules that compile against the default features, and `tests/integration_daemon.rs` declares the ones that reach symbols gated behind `full-runtime` - daemon, bridge, ACP and MCP. Only the second carries `required-features`, so a run that leaves `full-runtime` off never builds it, and `--test integration` alone links a library without axum, sqlx, hyper and rustls. Editing a test therefore rebuilds one target instead of both.
