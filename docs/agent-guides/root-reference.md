...
`mise run cargo:cache:status` queries the socket from `cargo-local.sh --print-env`, not sccache's default state, and reports wrapper configuration separately from cache mode, whether that configured server is healthy, cold, unreachable, or accompanied by safely identifiable orphan servers, and its request, hit, miss, and non-cacheable counts plus the physical cache path, birth time, and size. The configured 30 GiB per-server budget and `clean:caches` 100 GiB host guard solve different problems: one healthy server limits its own view, while leaked independent servers each enforce a separate 30 GiB view over the shared directory and can collectively drive disk use past one server's limit. Keep both values unless measured multi-server growth proves a safer replacement. Before a forced or over-threshold removal, `clean:caches` stops the configured server, writes a bounded 100-entry attribution record to the ignored `.cache/diagnostics/sccache-cleanup.jsonl`, and only then removes the cache; a failed stop or audit preserves the cache, while dry-run prints the same decision and writes nothing. This is the only repository cleanup path that removes sccache data, and the actor behind the historical deletion is unrecoverable.

Use this decision tree instead of treating hit rate as the goal:

- Healthy and warm: run the normal focused `mise` check or test task. A same-lane no-op rerun should be Cargo-fingerprint-fast and may invoke no compiler, so sccache has nothing to hit.
- Healthy and cold: keep the current lane and run the normal command you need. Deleted entries cannot be restored; ordinary compilation creates fresh reusable artifacts. There is no automatic warm-up because compiling unrelated surfaces spends more time than it saves.
