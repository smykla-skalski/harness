...
1. Find the merge base of the intended PR base and proposed branch head, then compute the proposed PR diff from that merge base to the branch head. Count additions plus deletions only for text files eligible for GitHub Copilot code review under its [documented excluded-file rules](https://docs.github.com/en/copilot/reference/review-excluded-files). Report the complete diff and separately report the generated, vendored, lockfile, snapshot, binary, or other mechanical volume excluded from the review budget; never use exclusions to hide an oversized authored diff or separate required derived output from its source.
