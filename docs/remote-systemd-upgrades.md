...
On a host with the Harness repo checked out, `mise run daemon:remote:deploy` builds and activates the daemon release set and then runs exactly this controller upgrade against the activated binary. Pass flags after a `--` separator so `mise` forwards them: `mise run daemon:remote:deploy -- --dry-run` skips the build and activation and only reports the controller transaction without mutating anything, and `mise run daemon:remote:deploy -- --unit NAME` targets a non-default unit. Run it as the normal install user; only the controller step self-elevates with `sudo`, so running the whole task as root would change `$HOME` and the default candidate path. It never rotates the controller: a newer `harness-systemd` still needs the `install` rotation described next.
...
Process attestation requires systemd 244 or newer because Harness verifies the manager-provided `STATE_DIRECTORY` value and rejects unit or environment-file attempts to override or remove it. Companion routing requires systemd 247 or newer because its managed unit uses `LoadCredential=` and the `%d` credential-directory specifier; installation checks that minimum before writing files.
