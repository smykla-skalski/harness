...
- Superseded by the cross-worktree correction: sccache 0.17 normalizes Rust working-directory, source, and compiler-argument paths through `SCCACHE_BASEDIRS`. Harness now supplies every registered source root and deterministic target lane while retaining format-v2 copy-on-write lane seeding as the independent Cargo artifact cache.
