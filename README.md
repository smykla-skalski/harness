...
Authoritative DNS-01 visibility uses provider-neutral runtime controls: `HARNESS_REMOTE_ACME_DNS_VISIBILITY_TIMEOUT_SECONDS` (default `900`), `HARNESS_REMOTE_ACME_DNS_VISIBILITY_POLL_SECONDS` (default `5`), and `HARNESS_REMOTE_ACME_DNS_VISIBILITY_STABLE_POLLS` (default `3`). The Aftermarket provider currently uses this check for both challenge presentation and cleanup. A state is accepted only after every authoritative nameserver IP reports the expected result for the configured number of consecutive polls. The 15-minute default covers the longer authoritative deletion propagation observed during live Aftermarket cleanup.
...
`mise run remote-daemon:public-acme-e2e` is the opt-in Linux proof against the Let's Encrypt staging directory. It requires passwordless `sudo` for granting `CAP_NET_BIND_SERVICE` to a disposable test binary, free ports 80 and 443, an Aftermarket-managed base domain, and these environment variables: `HARNESS_TEST_PUBLIC_ACME_BASE_DOMAIN`, `HARNESS_TEST_PUBLIC_ACME_IPV4`, `HARNESS_TEST_PUBLIC_ACME_EMAIL`, `AFTERMARKET_ZONE_NAME`, `AFTERMARKET_API_KEY`, and `AFTERMARKET_API_SECRET`. The task leases a unique A record for each of TLS-ALPN-01, HTTP-01, and DNS-01 as one guarded batch, verifies the issued certificate chain against a test-only bundle of the active Let's Encrypt staging roots, validates the leaf metadata and authenticated HTTPS/WSS behavior, then removes every lease and waits for stable authoritative absence. The ACME directory is pinned to staging so this task cannot consume production issuance limits, and the staging roots are never added to product or system trust stores.
