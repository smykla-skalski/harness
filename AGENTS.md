...
A `replay` task ends only when clean local `main` and the session worktree point to the same commit, with any `upstream/main` difference reported. If completed local replay commits block the post-merge PR fast-forward, rebase and re-sign only that local range onto merged `upstream/main`, then wait for the user to push it; never cherry-pick the PR commit on top of the local range. Successful `pr` delivery ends only after the user merges, local `main`, `upstream/main`, and the clean reusable worktree match, and stale upstream tracking is removed; a closed-unmerged PR uses the guide's explicit undelivered terminal state. The agent never merges the PR.
