...
#[must_use]
pub fn image_mime_for_path(path: &str) -> Option<ReviewImageMime> {
    ...
    match ext {
        "png" => Some(ReviewImageMime::Png),
        "jpg" | "jpeg" => Some(ReviewImageMime::Jpeg),
        "gif" => Some(ReviewImageMime::Gif),
        "svg" => Some(ReviewImageMime::Svg),
        _ => None,
    }
}
...
///
/// # Errors
/// Returns `base64::DecodeError` when `encoded` is not valid base64.
...
mod tests {
    ...
    fn mime_type_strings_match_iana() {
        assert_eq!(ReviewImageMime::Png.mime_type(), "image/png");
        assert_eq!(ReviewImageMime::Jpeg.mime_type(), "image/jpeg");
        assert_eq!(ReviewImageMime::Gif.mime_type(), "image/gif");
        assert_eq!(ReviewImageMime::Svg.mime_type(), "image/svg+xml");
    }
