...
#[cfg(test)]
mod lease_tests;
// Not plain `#[cfg(test)]`: root's own `external::sync_tests`/`external::tests`
// (which stayed in root, unmoved) build a `TaskBoardStore` and drive it
// through `TaskBoardSyncStore`/`TaskBoardExternalCreateStore` directly, and a
// crate boundary means root's own test build never sees this crate's
// `cfg(test)` items, only its `test-support`-gated ones (root's
// `[dev-dependencies]` already enables that feature on this crate).
...
pub use batch::{
    sync_external_tasks, sync_external_tasks_scoped, sync_external_tasks_scoped_with_recovery,
};
