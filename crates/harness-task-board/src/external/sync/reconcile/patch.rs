use crate::external::{ExternalTask, ExternalTaskRef, execution_repository_for_task};
...
pub(in crate::external::sync) fn reconciliation_patch(
    item: &TaskBoardItem,
    task: &ExternalTask,
    prefer_remote: bool,
...
fn reconcile_pull_request_identity(
    ...
    let author = task
        .pr_author
        .as_ref()
        .filter(|_| item.workflow.pr_author.is_none());
    ...
    if let Some(author) = author {
        workflow.pr_author = Some(author.clone());
    }
    patch.workflow = Some(workflow);
}
