...
use crate::external::{
    ExternalProvider, ExternalSyncField, ProviderExclusionAuditContext,
...
use crate::types::{ExternalRef, TaskBoardItem, TaskBoardStatus};
use harness_kernel::errors::{CliError, CliErrorKind};

#[derive(Debug, Clone)]
pub struct TaskBoardSyncItemSnapshot {
...
pub enum TaskBoardSyncCoordinatorFenceDecision {
    Current,
    Cancelled(CliError),
}

#[async_trait]
pub trait TaskBoardSyncCoordinatorFence: Send + Sync {
    async fn check(&self) -> Result<TaskBoardSyncCoordinatorFenceDecision, CliError>;
}

#[async_trait]
pub trait TaskBoardExternalCreateStore: Send + Sync {
...
#[async_trait]
pub trait TaskBoardSyncStore: TaskBoardExternalCreateStore {
    async fn list_items(
        &self,
