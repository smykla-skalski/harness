...
#[derive(Default)]
pub struct ExternalCreateRecoveryPlan {
    operations: Vec<ExternalSyncOperation>,
    follow_ups: Vec<TaskBoardExternalCreateIntent>,
...
#[derive(Default)]
pub struct ExternalCreateScopeRecovery {
...
impl PreparedExternalCreateRecovery {
    ...
    pub fn follow_ups(&self) -> &[TaskBoardExternalCreateIntent] {
        &self.follow_ups
    }
...
impl ExternalCreateRecoveryPlan {
    pub fn take_operations(&mut self) -> Vec<ExternalSyncOperation> {
        mem::take(&mut self.operations)
    }

    pub fn take_follow_ups(&mut self) -> Vec<TaskBoardExternalCreateIntent> {
    ...
    pub fn into_blocked(self, error: CliError) -> ExternalSyncBatch {
        let intents = self
            .scopes
            .into_iter()
