...
/// Validate a remote assignment's completed-result handoff back to ordinary
/// ownership.
///
/// # Errors
/// Returns [`TaskBoardWorkflowExecutionValidationError::InvalidField`] if the
/// handoff does not own the exact active attempt, clears ownership beyond
/// that one target, or does not carry exactly one completed coordinator
/// result; also returns any error the underlying attempt, execution, or
/// workflow validation raises.
pub fn validate_task_board_remote_result_handoff(
...
/// Validate a remote assignment's failure handoff back to ordinary ownership.
///
/// # Errors
/// Returns [`TaskBoardWorkflowExecutionValidationError::InvalidField`] if the
/// handoff does not own the exact active attempt, changes evidence beyond the
/// retry or human-required outcome it settles into, or pairs an execution
/// state with an attempt state that retry handling does not recognize; also
/// returns any error the underlying attempt, execution, or workflow
/// validation raises.
pub fn validate_task_board_remote_failure_handoff(
    current: &TaskBoardWorkflowExecutionRecord,
    updated: &TaskBoardWorkflowExecutionRecord,
    current_attempt: &TaskBoardExecutionAttemptRecord,
