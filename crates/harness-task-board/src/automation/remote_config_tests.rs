...
use crate::TaskBoardOrchestratorWorkflow;
use crate::remote_spki_pin::encode;
...
fn config(reference: &str) -> TaskBoardExecutionHostConfig {
    TaskBoardExecutionHostConfig {
        ...
        certificate_fingerprint: encode([0x11; 32]),
...
fn repository_routing_references_only_configured_trust_anchors() {
    ...
    validate_remote_execution_configuration(from_ref(&host), from_ref(&repository))
...
fn host_configs_require_canonical_unique_operator_identity() {
    ...
    validate_execution_host_configs(from_ref(&first)).expect("one host");
...
#[expect(
    ...
    reason = "round-trips every assignment and host state label through decode/as_str"
)]
