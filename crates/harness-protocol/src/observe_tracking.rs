...
impl ToolUseWindow {
    // Root's test module reads this across the crate boundary now, so it
    // can't stay `pub(crate)` the way it was before the move.
    ...
    // Same reason as `LIMIT`: dropped the `#[cfg(test)]` this and `len` had
    // before the move. cfg(test) is per-crate, so root's test build never
    // enables it for a dependency; root still gates its re-export of the
    // type itself to test builds only.
    #[must_use]
