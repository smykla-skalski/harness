//! GitHub project/automation config, relocated here unchanged from
//! `harness-task-board::github_config`. Pure data plus pure string-matching
//! logic (`ProtectedPathRule::matches`, `GitHubRequestedReviewers::normalized_*`);
//! `harness-task-board` re-exports every name below at the same path.

use serde::{Deserialize, Serialize};

use super::automation_settings::TaskBoardRepositoryAutomationConfig;
...
/// Already local to this crate (`harness_protocol::daemon::reviews::github_merge_method`,
/// moved there by #1056 for the same reverse-dependency reason this module
/// moves now): `harness-reviews`' wire types embed it directly and
/// `harness-protocol` cannot depend on `harness-task-board`, which itself
/// depends on `harness-protocol`, to reach it. Re-exported here unchanged so
/// `harness-task-board::github_config` keeps resolving the same name at the
/// same path.
pub use crate::daemon::reviews::github_merge_method::GitHubMergeMethod;
...
// `harness-task-board::github_config`'s own `#[path]` test module, exercised
