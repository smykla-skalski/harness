//! Task-board git runtime and token-sync config, relocated here unchanged
//! from `harness-task-board::runtime_config`. Pure data plus pure inherent
//! methods (secret stripping, override merging); `harness-task-board`
//! re-exports every name below at the same path.
...
// `harness-task-board::runtime_config`'s own test module, exercised through
// the re-export below.
