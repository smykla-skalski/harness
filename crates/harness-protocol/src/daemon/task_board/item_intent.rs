//! Pull request intent carried by a task-board ticket, relocated here
//! unchanged from `harness-task-board::item_intent`. Pure data: every
//! inherent method below is a `const fn` over the type's own bits, with no
//! dependency beyond `serde`/`utoipa`. `harness-task-board` re-exports both
...
// Existing coverage for these types stays in `harness-task-board::item_intent`'s
