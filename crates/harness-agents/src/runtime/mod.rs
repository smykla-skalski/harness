...
//! lives in this crate's `acp` sibling module and speaks Agent Client
//! Protocol over stdio, with harness servicing `fs/*`, `terminal/*`, and
//! `session/request_permission` directly. `RuntimeKind` (Chunk 2) joins the
//! two ends. Prefer adding new agents as ACP descriptors in `acp::catalog`
//! rather than as new TUI shells here.
mod claude;
mod codex;
mod copilot;
pub mod event {
