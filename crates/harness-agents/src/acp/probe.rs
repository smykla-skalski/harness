...
use harness_workspace::workspace::{account_home_dir, dirs_home, normalized_env_value, utc_now};
...
impl ProbeCacheRefresh {
    ...
    pub fn publish(mut self, response: AcpRuntimeProbeResponse) {
...
#[must_use]
pub fn local_cached_probe_snapshot() -> Option<AcpRuntimeProbeResponse> {
...
/// Panics if the process-wide probe cache mutex is poisoned.
#[must_use]
pub fn cached_probe_snapshot_with(
...
pub fn spawn_local_probe_cache_refresh(refresh: ProbeCacheRefresh) {
...
// Both of the following are gated on `test-support` (not just `cfg(test)`)
// because `src/daemon/acp_probe.rs`'s own tests, compiled as part of the root
// crate, need these too; `cfg(test)` only activates for this crate's own test
// binary, never for a dependent crate's.

/// # Panics
/// Panics if the process-wide probe cache test lock is poisoned.
#[cfg(any(test, feature = "test-support"))]
pub fn lock_probe_cache_for_tests() -> MutexGuard<'static, ()> {
...
/// Panics if the process-wide probe cache mutex is poisoned.
#[cfg(any(test, feature = "test-support"))]
pub fn replace_probe_cache_for_tests(
    ...
    cache.entry = response.map(|response| ProbeCacheEntry {
        cached_at: Instant::now().checked_sub(age).unwrap_or_else(Instant::now),
        response,
    });
...
mod tests {
    ...
    use harness_protocol::managed_agents::acp::{AcpSessionConfiguration, AcpSpawnConfiguration};

    use super::*;
    use crate::acp::catalog::{DoctorProbe, tags};
