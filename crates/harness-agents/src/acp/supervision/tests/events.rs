...
use crate::acp::connection::SupervisorEventSink;
use crate::runtime::event::ConversationEventKind;
...
struct RecordingEmitter {
    transitions: Mutex<Vec<(WatchdogState, WatchdogState, Option<String>)>>,
}
...
async fn supervisor_event_sink_carries_terminal_transition_through_mpsc() {
    ...
    assert_eq!(batch.acp_id, "acp-test");
...
/// Asserts `kind` is a `ContextInjected` event with the given actor/summary,
/// factored out of its two callers so each stays a flat sequence of
/// assertions instead of tripping the cognitive-complexity budget on its own
/// match arm.
fn assert_context_injected(kind: &ConversationEventKind, actor: &str, summary: Option<&str>) {
    let ConversationEventKind::ContextInjected {
        actor: got_actor,
        summary: got_summary,
    } = kind
    else {
    ...
    assert_eq!(got_summary.as_deref(), summary);
}
...
fn supervisor_event_sink_emits_context_injected_with_actor_and_summary() {
    ...
    assert_context_injected(&batch.events[0].kind, "acp", Some("wake prompt accepted"));
}
...
fn supervisor_event_sink_emits_context_injected_without_summary() {
    ...
    let batch = rx
        ...
        .expect("context_injected batch must be admitted");
    assert_eq!(batch.acp_id, "acp-test");
    assert_context_injected(&batch.events[0].kind, "acp", None);
}
