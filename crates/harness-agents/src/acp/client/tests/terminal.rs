...
use crate::acp::client::{HarnessAcpClient, TERMINAL_DENIED, no_cancel};
...
fn terminal_wait_on_one_terminal_does_not_block_output_for_another() {
    ...
    File::create(&gate).expect("open slow gate");
    wait_thread
        .join()
        .expect("wait thread")
        .expect("wait terminal");
