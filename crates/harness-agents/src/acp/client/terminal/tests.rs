...
use crate::acp::client::{ClientCallCancel, ClientResult, TERMINAL_DENIED, no_cancel};
use crate::policy::DeniedBinaries;

#[track_caller]
fn ok<T, E: fmt::Debug>(result: Result<T, E>, context: &str) -> T {
    assert!(
        result.is_ok(),
...
/// Joins every slot-reservation thread and splits the results into accepted
/// terminal ids and denial error codes, asserting nothing itself so the
/// caller's own assertions stay the single source of test intent.
fn join_slot_reservations(
    handles: Vec<thread::JoinHandle<ClientResult<String>>>,
) -> (Vec<String>, Vec<i32>) {
    let mut accepted = Vec::new();
    let mut denied_codes = Vec::new();
    for handle in handles {
        let Ok(reservation) = handle.join() else {
            unreachable!("reservation thread");
        };
        match reservation {
            Ok(terminal_id) => accepted.push(terminal_id),
            Err(error) => denied_codes.push(error.code),
        }
    }
    (accepted, denied_codes)
}
...
fn terminal_slot_race_reserves_cap_under_concurrency() {
    ...
    let (reserved_ids, denied_codes) = join_slot_reservations(handles);

    assert_eq!(reserved_ids.len(), 2);
    assert_eq!(denied_codes.len(), 6);
    assert!(denied_codes.iter().all(|&code| code == TERMINAL_DENIED));
