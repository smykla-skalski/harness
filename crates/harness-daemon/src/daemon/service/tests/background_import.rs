use super::*;
use crate::daemon::db::DaemonDbImports;
...
#[test]
fn background_project_sync_surfaces_a_poisoned_db_lock() {
    use std::sync::{Arc, Mutex, OnceLock};
    ...
    with_temp_project(|project| {
        ...
        .expect_err("a poisoned lock must not report success");
        assert!(
            error.to_string().contains("poisoned"),
            "expected the poisoned lock to be named, got {error}"
        );
