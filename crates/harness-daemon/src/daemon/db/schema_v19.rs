...
pub(super) fn run(conn: &Connection) -> Result<(), CliError> {
    if column_exists(conn, "policy_workspace", "manual_ocr_paste_canvas_deleted")?
        && column_exists(conn, "policy_canvases", "is_manual_ocr_paste_canvas")?
    {
    ...
    if !column_exists(conn, "policy_workspace", "manual_ocr_paste_canvas_deleted")? {
        add_column_if_missing(
            conn,
            "policy_workspace",
            "manual_ocr_paste_canvas_deleted",
            "ALTER TABLE policy_workspace
             ADD COLUMN manual_ocr_paste_canvas_deleted INTEGER NOT NULL DEFAULT 0",
            "add v19 manual OCR workspace tombstone",
        )?;
    }
    if !column_exists(conn, "policy_canvases", "is_manual_ocr_paste_canvas")? {
        add_column_if_missing(
            conn,
            "policy_canvases",
            ...
            "add v19 manual OCR canvas identity",
        )?;
    }
    conn.execute(
    ...
    .map_err(|error| db_error(format!("backfill v19 manual OCR canvas: {error}")))?;
    stamp_schema_version(conn)
}
...
fn stamp_schema_version(conn: &Connection) -> Result<(), CliError> {
    ...
    .map(|_| ())
    .map_err(|error| db_error(format!("stamp schema v19: {error}")))
}
