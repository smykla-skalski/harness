// Companion to `harness-db-schema`'s `schema_v43_legacy_target_tests`: that
// crate's copy covers the same v43 upgrade path against a raw-SQL fixture,
// this one covers it against a workflow execution built through the real
// async dispatch/offer/claim path. It stays here rather than moving with the
// rest of the v43 test family because it needs
// `db::task_board::remote_assignment_test_support`'s `ControllerFixture`,
...
async fn v43_upgrade_marks_and_consumes_only_the_exact_legacy_starting_generation() {
    ...
    harness_db_schema::schema_v43::restore_legacy_v40_for_test(sync.connection());
    sync.connection()
        .execute(
