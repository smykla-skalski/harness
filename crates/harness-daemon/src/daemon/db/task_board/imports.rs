...
async fn import_task_board_snapshot(
    ...
    let mut transaction = db
        ...
        .await?;
    if let Some(marker) = verify::load_marker_in_tx(&mut transaction, source_kind).await? {
        verify::verify_existing_marker(&marker, snapshot)?;
        transaction
            .commit()
            .await
    ...
    verify::ensure_import_target_empty(&mut transaction).await?;
    let lane_writes = insert_snapshot(&mut transaction, snapshot, runtime_config).await?;
    verify::verify_snapshot(&mut transaction, snapshot, runtime_config).await?;
