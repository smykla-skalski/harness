...
pub(super) async fn record_task_board_remote_artifact_fetch_response(
    db: &AsyncDaemonDb,
    ...
    let content = response
        .validate(request)
        .map_err(|error| db_error(format!("validate remote artifact fetch response: {error}")))?;
    ...
    .await?;
    let stored =
        store_or_reuse_artifact_in_tx(&mut transaction, &input, &request.relative_path, existing)
            .await?;
    transaction
        .commit()
        .await
