...
impl AsyncTimelineWindowQueries for AsyncDaemonDb {
    async fn load_session_timeline_window(
        ...
        if let Some(response) = unchanged_timeline_window_response(
            self,
            session_id,
            request,
            state.revision,
        ...
        let window_start =
            timeline_window_start(self, session_id, request, entry_count, limit).await?;
        let entries = load_timeline_window_entries(
            self,
            session_id,
            request,
            entry_count,
...
async fn unchanged_timeline_window_response(
    ...
    Ok(Some(TimelineWindowResponse {
        ...
        has_newer: false,
        oldest_cursor: load_timeline_cursor_at_offset(
            db,
            session_id,
            latest_window_end.checked_sub(1),
        )
        .await?,
        newest_cursor: load_timeline_cursor_at_offset(db, session_id, Some(0)).await?,
        entries: None,
        unchanged: true,
    }))
}
...
async fn timeline_window_start(
    ...
    if let Some(before) = &request.before {
        return load_timeline_cursor_offset(db, session_id, before)
            .await
            .map(|offset| offset.map_or(entry_count, |offset| offset.saturating_add(1)));
    }
    if let Some(after) = &request.after {
        return load_timeline_cursor_offset(db, session_id, after)
            .await
...
async fn load_timeline_window_entries(
    ...
    if let Some(after) = &request.after {
        ...
        return load_timeline_entries_range(
            db,
            session_id,
            window_start,
