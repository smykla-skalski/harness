...
async fn stop_after_central_offer_prevents_offer_network_io() {
    ...
    let error = Box::pin(temp_env::async_with_vars(
        [(TOKEN_ENV, Some("authority-secret"))],
        async {
            Box::pin(controller.offer(&state.fixture.db, &state.fixture.request))
                .await
...
async fn stop_after_acceptance_prevents_claim_network_io() {
    ...
    let error = Box::pin(temp_env::async_with_vars(
        [(TOKEN_ENV, Some("authority-secret"))],
        async {
            Box::pin(controller.claim(&state.fixture.db, &claim))
                .await
