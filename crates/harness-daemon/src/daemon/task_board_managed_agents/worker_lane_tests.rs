...
use super::test_support::{
    applied_task, codex_snapshot, seed_owner_session, seed_workspace_owner, test_http_state,
};
use super::{
...
#[tokio::test]
async fn reclaimed_worker_repairs_workspace_membership_before_claim_preflight() {
    let state = test_http_state();
    let db = state.async_db.get().cloned().expect("test async db");
    let mut applied = applied_task(AgentMode::Headless);
    let workspace_id = "workspace-reclaimed-worker";
    applied.session_id = None;
    applied.workspace_id = Some(workspace_id.into());
    applied.item.session_id = None;
    applied.item.workspace_id = Some(workspace_id.into());
    let intent_id = "dispatch-intent-workspace-reclaimed";
    let worker_id = managed_worker_id(&applied, intent_id);
    seed_workspace_owner(&db, workspace_id).await;
    let mut snapshot = codex_snapshot(CodexRunStatus::Running, &worker_id);
    snapshot.run_id.clone_from(&worker_id);
    snapshot.board_item_id = Some(applied.board_item_id.clone());
    snapshot.task_id = Some(applied.work_item_id.clone());
    snapshot.workflow_execution_id = applied.item.workflow.execution_id.clone();
    ...
    db.save_codex_run(&snapshot)
        .await
        .expect("persist interrupted worker start");

    start_worker_for_applied_task(&state, &applied, intent_id, "stale-claim")
        .await
        .expect("reclaim worker before claim validation");
    ...
    .fetch_one(db.pool())
    .await
    .expect("count repaired memberships");
    assert_eq!(member_count, 1);
    let owner: Option<String> =
        sqlx::query_scalar("SELECT workspace_id FROM codex_runs WHERE run_id = ?1")
            .bind(&worker_id)
            .fetch_one(db.pool())
            .await
            .expect("load repaired runtime owner");
    assert_eq!(owner.as_deref(), Some(workspace_id));
}
