...
const WORKER_GOLDEN: &str = "Work on task-board item 'Ship managed worker launch'.\n\nBoard item: board-1\nSession task: task-1\nPriority: High\nStatus: InProgress\n\nWorktree:\n/tmp/task-worktree\n\nSession id:\nsession-1\n\nManaged run id:\ncodex-dispatch-intent-1\n\nTags:\nbackend\n\nExternal refs:\ngithub:123 (https://github.example/issues/123)\n\nTask body:\nStart a real worker.\n\nLifecycle:\nImplement the requested work, keep changes scoped, and run the smallest relevant validation.\n1. Report progress with `harness task-board progress checkpoint --item-id board-1 --work-item-id task-1 --summary \"<summary>\" --progress <0-100>`.\n2. Submit with `harness task-board progress submit-for-review --item-id board-1 --work-item-id task-1 --summary \"<summary>\"`.\n3. If the work cannot continue, run `harness task-board progress block --item-id board-1 --work-item-id task-1 --reason \"<reason>\"` instead of stopping silently.\nRead the record back at any time with `harness task-board progress show --item-id board-1`. The controller also settles this item when the managed run completes and is the authoritative safety net.";
