...
/// Resolve the scope required by a remote HTTP route.
///
/// # Errors
/// Returns [`RemoteAuthError::MissingScopeContract`] when the route has no
/// remote scope contract.
pub fn remote_http_required_scope(
    route: &HttpApiRouteContract,
) -> Result<RemoteAccessScope, RemoteAuthError> {
    first_required_scope(remote_http_scopes(route))
}

/// Resolve the scope required by a remote WebSocket method.
///
/// # Errors
/// Returns [`RemoteAuthError::MissingScopeContract`] when the method has no
/// remote scope contract.
pub fn remote_ws_required_scope(method: &str) -> Result<RemoteAccessScope, RemoteAuthError> {
