...
use super::request::{ObserveActionKind, ObserveDumpRequest, ObserveFilter, ObserveScanRequest};
use crate::{compare, context_cmd, dump, scan};
...
/// Dispatch a scan-mode request to the concrete scan/maintenance action it
/// selects. The doctor mode has no analogue here: it stays on the root
/// crate's side of the boundary, since it depends on `crate::setup`, which
/// harness-observe cannot see.
///
/// # Errors
/// Returns an error when the request is missing a required argument for its
/// action, or when the underlying scan/maintenance/compare call fails.
pub fn execute_scan_mode(request: &ObserveScanRequest) -> Result<i32, CliError> {
...
/// # Errors
/// Returns an error when the session log cannot be located or read.
pub fn execute_dump_mode(request: &ObserveDumpRequest) -> Result<i32, CliError> {
