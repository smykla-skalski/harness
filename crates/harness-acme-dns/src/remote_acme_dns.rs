...
/// config enums; moved here because the runner and provider selector below
/// match on it directly, and `daemon::remote` now re-exports it under the
/// same path so its other callers (`db`, `transport`) keep compiling
/// unchanged.
