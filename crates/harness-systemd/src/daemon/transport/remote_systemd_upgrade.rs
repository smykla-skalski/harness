    ...
    fn execute(&self, _context: &AppContext) -> Result<i32, CliError> {
        ...
        if !self.confirm_data_loss {
            return Err(CliErrorKind::workflow_parse(
                "harness-systemd rollback requires --confirm-data-loss because it restores the previous database and discards newer writes"
                    .to_string(),
            )
            .into());
        }
...
pub(super) fn ensure_root() -> Result<(), CliError> {
    if uzers::get_current_uid() == 0 {
        ...
        Err(CliErrorKind::workflow_io(
            "harness-systemd lifecycle mutations require root".to_string(),
        )
        .into())
    }
}
