    ...
    RunSystemctl: Fn(&[String]) -> Result<RemoteSystemdCommandOutput, CliError>,
{
    ...
    .and_then(|()| {
        arm.controller_sha256 = Some(sha256_file(&plan.recovery_controller_path())?);
        write_recovery_units(plan)
    });
...
where
    RunSystemctl: Fn(&[String]) -> Result<RemoteSystemdCommandOutput, CliError>,
{
    sync_armed_daemon_digest(plan, arm)?;
