...
fn recovery_timer_drop_in_fails_before_disable() {
    ...
    let runner = |args: &[String]| {
        ...
        if command(args) == Some("show") && args.last().is_some_and(|name| is_timer(name)) {
...
fn timer_still_enabled_after_disable_preserves_artifacts() {
    ...
    let runner = |args: &[String]| {
        ...
        if command(args) == Some("show")
            && disabled.get()
            && args.last().is_some_and(|name| is_timer(name))
        {
            return Ok(show_output(
                "loaded",
                "inactive",
...
fn symlinked_recovery_unit_is_never_removed() {
    ...
    symlink("/etc/passwd", &service_path).expect("symlink recovery service");
...
impl CleanupFixture {
    fn empty() -> Self {
        ...
        fs::create_dir(&unit_directory).expect("systemd directory");
    ...
    fn with_artifacts() -> Self {
        ...
        fixture.write_artifacts(&service, &timer);
        fixture
    }

    fn write_artifacts(&self, service: &str, timer: &str) {
        fs::write(self.plan.recovery_service_path(), service).expect("recovery service");
        fs::write(self.plan.recovery_timer_path(), timer).expect("recovery timer");
        let controller = self.plan.store_path.join(RECOVERY_CONTROLLER_FILE);
        fs::write(&controller, b"controller").expect("recovery controller");
        fs::set_permissions(&controller, Permissions::from_mode(0o700))
