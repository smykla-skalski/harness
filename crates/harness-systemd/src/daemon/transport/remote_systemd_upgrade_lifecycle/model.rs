...
pub(super) const RECOVERY_ARM_VERSION: u32 = 3;
pub(super) const LEGACY_RECOVERY_ARM_VERSION: u32 = 2;
...
impl RecoveryArm {
    pub(super) fn validate(&self) -> Result<(), CliError> {
        match (self.arm_version, self.controller_sha256.as_deref()) {
            ...
            (LEGACY_RECOVERY_ARM_VERSION, None) => {}
            (LEGACY_RECOVERY_ARM_VERSION, Some(_)) => {
                return Err(io_error(
                    "legacy systemd recovery arm v2 cannot carry a controller digest",
                ));
            }
            (version, _) => {
