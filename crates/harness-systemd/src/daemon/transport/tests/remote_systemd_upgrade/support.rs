...
pub(super) const CONTROLLER_BINARY: &str = "#!/bin/sh\necho 'harness-systemd 48.0.0'\n";
...
impl UpgradeFixture {
    pub(super) fn new() -> Self {
        ...
        write_executable(&controller, CONTROLLER_BINARY);
        ...
        .expect("establish controller and daemon release pair");
        Self {
            _temp: temp,
...
struct ScriptedSystemdState {
    ...
    daemon_failures: DaemonFailures,
    reload_failures: ReloadFailures,
    permit_behavior: PermitBehavior,
    panics: PanicBoundaries,
    late_panics: LatePanicBoundaries,
...
impl<'a> ScriptedSystemd<'a> {
    ...
    pub(super) fn run(&self, args: &[String]) -> Result<RemoteSystemdCommandOutput, CliError> {
        ...
        match args.first().map(String::as_str) {
            ...
            Some("enable") => Self::handle_enable(args, &mut state)?,
            Some("disable") => self.handle_disable(args, &mut state)?,
            Some("show") => return Ok(self.show_output(args, &mut state)),
            Some("stop") => self.handle_stop(&mut state)?,
    ...
    fn handle_enable(args: &[String], state: &mut ScriptedSystemdState) -> Result<(), CliError> {
        if recovery_output::is_timer_command(args) && state.timer_failures.enable {
            return Err(CliErrorKind::workflow_io(
                "forced recovery timer enable failure".to_string(),
            )
            .into());
        }
        Self::enable(args, state);
        Ok(())
    }
    ...
    fn handle_disable(
        ...
        if recovery_output::is_timer_command(args) && state.timer_failures.disable {
            return Err(CliErrorKind::workflow_io(
                "forced recovery timer disable failure".to_string(),
            )
            .into());
        }
        if !recovery_output::is_timer_command(args) && state.daemon_failures.fail_daemon_disable {
            return Err(
                CliErrorKind::workflow_io("forced daemon disable failure".to_string()).into(),
            );
        }
        self.disable(args, state);
        Ok(())
    }

    fn handle_stop(&self, state: &mut ScriptedSystemdState) -> Result<(), CliError> {
        state.service.active = false;
        self.write_cgroup_populated(false);
        if state.daemon_failures.fail_stop_after_inactive {
            state.daemon_failures.fail_stop_after_inactive = false;
            return Err(CliErrorKind::workflow_io(
        ...
        assert!(
            !state.late_panics.panic_on_stop,
            "simulated crash after service stop"
        );
        Ok(())
    }
}
...
fn mutate_candidate_state(database: &Path, state: &Path) -> Result<(), CliError> {
    let connection = Connection::open(database).map_err(|error| test_db_error(&error))?;
    connection
        .set_db_config(DbConfig::SQLITE_DBCONFIG_NO_CKPT_ON_CLOSE, true)
        .map_err(|error| test_db_error(&error))?;
    connection
        .execute_batch(
