...
fn bidirectional_reserve_includes_both_complete_generations() {
    let fixture = UpgradeFixture::new();
    let (first, second) = snapshot_complete_generations(&fixture);
    let (expected_bytes, expected_inodes) =
        expected_bidirectional_capacity(&fixture.operation, &first, &second);

    reserve_bidirectional_restore_capacity_for_tests(&fixture.operation, &first, &second)
        .expect("reserve both rollback directions");

    assert_bidirectional_reserves(&fixture.operation, expected_bytes, expected_inodes);
    release_restore_capacity_for_tests(&fixture.operation).expect("release rollback capacity");
    assert_bidirectional_reserves_released(&fixture.operation);
}

fn snapshot_complete_generations(fixture: &UpgradeFixture) -> (PathBuf, PathBuf) {
    fs::create_dir_all(&fixture.operation.store_path).expect("create transaction store");
...
fn expected_bidirectional_capacity(
    ...
    let file_refs = file_sources
        ...
        .map(PathBuf::as_path)
        .collect::<Vec<_>>();
    let expected = required_restore_capacity_for_tests(plan, &state_refs, &file_refs)
        .expect("combined generation capacity");
    let expected_inodes = required_restore_inodes_for_tests(plan, &state_refs, &file_refs)
        .expect("combined generation inode capacity");
    (expected, expected_inodes)
}

fn assert_bidirectional_reserves(
    plan: &RemoteSystemdOperationPlan,
    expected: (u64, u64),
    expected_inodes: (u64, u64),
) {
    assert_reserve(&plan.store_path.join("state-restore-reserve"), expected.0);
    assert_reserve(
        &plan
            .binary_path
            .parent()
    ...
    assert_inode_reserve(
        &plan.store_path.join("state-restore-inode-reserve"),
        expected_inodes.0,
    );
    assert_inode_reserve(
        &plan
            ...
            .parent()
            .expect("binary parent")
            .join(format!(".harness-{}-binary-inode-reserve", plan.unit)),
        expected_inodes.1,
    );
}

fn assert_bidirectional_reserves_released(plan: &RemoteSystemdOperationPlan) {
    assert!(!plan.store_path.join("state-restore-reserve").exists());
    assert!(!plan.store_path.join("state-restore-inode-reserve").exists());
    assert!(
        !plan
            .binary_path
            .parent()
