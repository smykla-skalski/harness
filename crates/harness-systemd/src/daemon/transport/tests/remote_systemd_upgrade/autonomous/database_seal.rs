        ...
        let _ = upgrade_remote_systemd_with(
            ...
            &|plan, expected, run| {
                if installed_is_candidate(&fixture.binary) && runner.starts() == 2 {
                    ...
                    assert_eq!(arm["arm_version"], 3);
                    assert_eq!(
                        arm["controller_sha256"]
                            .as_str()
...
fn committing_phase_with_pending_generation_rolls_back_before_rotation() {
    ...
    let crash = catch_unwind(AssertUnwindSafe(|| {
        let _ = upgrade_remote_systemd_with(
            ...
            &|plan, expected, run| {
                assert!(
                    !(installed_is_candidate(&fixture.binary) && runner.starts() == 2),
                    "simulated crash before committing phase"
                );
                runner.verify(plan, expected, run)
            },
        );
...
fn committed_explicit_rollback_failure_restores_the_displaced_generation() {
    ...
    let arm = read_recovery_arm(&fixture);
    assert_rollback_crash_arm(&crash, &arm);
    ...
    let failed_database = fixture
        ...
        .join(format!("failed-current-{transaction_id}"))
        .join("daemon/external/harness.db");
    assert_displaced_generation_restored(&fixture, &report, &failed_database);
}
...
fn failed_committed_fallback_stays_committing_and_retries_after_evidence_is_repaired() {
    ...
    let error = recover_remote_systemd_with(
        ...
        &run_systemctl,
        &|plan, expected, run| {
            verify_with_corrupted_fallback(
                &fixture,
                &runner,
                &retained_binary,
                &corrupt_once,
    ...
    assert_corrupt_fallback_remains_armed(&fixture, &runner, &error);
    ...
    let report = recover_remote_systemd_with(
        ...
        &run_systemctl,
        &|plan, expected, run| {
            verify_committed_target_failure(&fixture, &runner, plan, expected, run)
        },
    )
    .expect("retry committed fallback");

    assert_fallback_retry_completed(&fixture, &report);
}

fn assert_rollback_crash_arm(crash: &Result<(), Box<dyn Any + Send>>, arm: &Value) {
    assert!(crash.is_err());
    assert_eq!(arm["operation"], "rollback");
    assert_eq!(arm["phase"], "committing");
}

fn assert_displaced_generation_restored(
...
fn assert_corrupt_fallback_remains_armed(
    fixture: &UpgradeFixture,
...
fn assert_fallback_retry_completed(fixture: &UpgradeFixture, report: &RemoteSystemdRecoveryReport) {
    assert_eq!(report.outcome, RemoteSystemdRecoveryOutcome::RolledBack);
    assert_eq!(
        fs::read_to_string(&fixture.binary).expect("restored binary"),
        OLD_BINARY
...
fn verify_with_corrupted_fallback<RunSystemctl>(
    fixture: &UpgradeFixture,
    runner: &ScriptedSystemd<'_>,
...
{
    if !installed_is_candidate(&fixture.binary) {
        return runner.verify(plan, expected, run);
    }
    ...
    Err(CliErrorKind::workflow_io("forced committed target health failure".to_string()).into())
}

fn verify_committed_target_failure<RunSystemctl>(
    fixture: &UpgradeFixture,
    runner: &ScriptedSystemd<'_>,
    plan: &RemoteSystemdOperationPlan,
    expected: &str,
