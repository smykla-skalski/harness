...
fn exact_inhibitor_install_is_idempotent() {
    ...
    assert_inhibitor_metadata(&expected);
    assert_drop_in_directory_metadata(expected.parent().expect("drop-in directory"));
}

fn assert_inhibitor_metadata(path: &Path) {
    let metadata = fs::symlink_metadata(path).expect("inhibitor metadata");
...
fn assert_drop_in_directory_metadata(path: &Path) {
    let directory_metadata = fs::symlink_metadata(path).expect("drop-in directory metadata");
