...
//! - `HARNESS_FEATURE_TASK_BOARD_PROMPT_OVERRIDES=1` lets
//!   `HARNESS_TASK_BOARD_PROMPTS_FILE` replace the prompts agents run with.
//!   Off by default so shipped prompts stay byte-identical. Implemented in
//!   `harness_task_board::prompt_config`, its only consumer, not here.
