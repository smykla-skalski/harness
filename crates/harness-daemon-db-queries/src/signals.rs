...
use harness_protocol::session::{SessionSignalRecord, SessionSignalStatus, SessionState};
...
///
/// `pub`: `sync_signal_index`, `load_signals`, and
/// `session_has_shared_runtime_signal_dir` were already `pub` on `DaemonDb`
/// before this trait existed, and every method needs it now that this trait
...
#[must_use]
pub fn derive_effective_signal_status(
