...
pub(super) async fn insert_signal(
    ...
    .bind(&target.runtime)
    .bind(&signal_json)
    .bind(target.source_session_id.as_deref().unwrap_or(workspace_id))
    .bind(target.source_agent_id.as_deref().unwrap_or(member_id))
    .bind(target.runtime_session_id.as_deref().unwrap_or(workspace_id))
    .bind(&target.project_dir)
    .bind(&source_digest)
    .bind(&now)
