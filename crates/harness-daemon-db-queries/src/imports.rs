...
///
/// Shared with [`DaemonDbSessionResync::apply_prepared_session_resync`] below.
///
/// # Errors
...
#[derive(Debug)]
pub struct PreparedSessionResync {
    pub resolved: daemon_index::ResolvedSession,
    log_entries: Vec<SessionLogEntry>,
    task_checkpoints: Vec<PreparedTaskCheckpointImport>,
    signals: Vec<SessionSignalRecord>,
    activities: Vec<AgentToolActivitySummary>,
    conversation_events: Vec<PreparedConversationEventImport>,
}

#[derive(Debug)]
pub struct PreparedTaskCheckpointImport {
    checkpoints: Vec<TaskCheckpoint>,
}

/// Fields are `pub`: `harness-daemon`'s own test suite constructs this
/// directly rather than always going through [`prepare_runtime_transcript_resync`].
#[derive(Debug)]
pub struct PreparedRuntimeTranscriptResync {
    pub session_id: String,
    pub agents: Vec<PreparedAgentTranscriptResync>,
}

pub trait DaemonDbSessionResync {
...
pub fn prepare_session_resync(session_id: &str) -> Result<PreparedSessionResync, CliError> {
...
pub fn prepare_session_import_from_resolved(
    resolved: &daemon_index::ResolvedSession,
...
/// # Errors
/// Returns [`CliError`] on discovery, I/O, or parse failures.
pub fn prepare_runtime_transcript_resync(
    session_id: &str,
    runtime_name: &str,
