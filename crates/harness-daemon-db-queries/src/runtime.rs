...
/// Row shape of the minimal freshness state
/// `RuntimeSnapshotQueries::agent_tui_live_refresh_state` guards live-refresh
/// persists with. Lives here (not `harness-daemon`) since the trait method
/// returning it moved with the rest of `RuntimeSnapshotQueries`.
...
#[must_use]
pub fn codex_mode_as_str(mode: CodexRunMode) -> &'static str {
    match mode {
...
/// # Errors
/// Returns an error string when `value` is not a known codex run mode.
pub fn codex_mode_from_str(value: &str) -> Result<CodexRunMode, String> {
    match value {
...
#[must_use]
pub fn codex_status_as_str(status: CodexRunStatus) -> &'static str {
    match status {
...
/// # Errors
/// Returns an error string when `value` is not a known codex run status.
pub fn codex_status_from_str(value: &str) -> Result<CodexRunStatus, String> {
    match value {
