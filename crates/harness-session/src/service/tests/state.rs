use super::*;
use crate::types::{ManagedAgentRef, RuntimeSessionId, SessionAgentId};
...
fn record_runtime_session_title_stores_and_deduplicates() {
    let now = "2026-04-12T00:00:00Z";
    let managed_agent = ManagedAgentRef::acp("acp-1");
    ...
    assert!(
        !super::super::apply_record_runtime_session_title(
            &mut state,
            &ManagedAgentRef::acp("acp-missing"),
            "Other",
            now,
        ),
...
fn register_runtime_session_preserves_identity_classes() {
    let now = "2026-04-12T00:00:00Z";
    let managed_agent = ManagedAgentRef::tui("tui-123");
    ...
    assert_eq!(
        ...
        Some(RuntimeSessionId::from("runtime-123"))
    );
}
