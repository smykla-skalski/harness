...
fn observer_transfer_leader_succeeds_when_current_leader_is_unresponsive() {
    with_temp_project(|project| {
        ...
        storage::update_state(&layout_timeout, |state| {
            let stale_timestamp = (Utc::now() - Duration::seconds(600)).to_rfc3339();
            let leader = state.agents.get_mut(&leader_id).expect("leader");
            leader.last_activity_at = Some(stale_timestamp.clone());
            state.last_activity_at = Some(stale_timestamp);
            Ok(())
        })
        .expect("mark stale");
