...
#[must_use]
pub fn task_not_found(task_id: &str) -> CliError {
    CliErrorKind::session_not_active(format!("task '{task_id}' not found")).into()
}

/// # Errors
/// Returns [`CliError`] when `progress` exceeds 100.
pub fn ensure_valid_progress(progress: u8) -> Result<(), CliError> {
    if progress > 100 {
...
#[must_use]
pub fn next_task_id(tasks: &BTreeMap<String, WorkItem>) -> String {
...
#[must_use]
pub fn next_available_agent_id(
    runtime_name: &str,
...
#[must_use]
pub fn generate_checkpoint_id(task_id: &str) -> String {
    format!("{task_id}-cp-{}", Utc::now().format("%Y%m%d%H%M%S%f"))
}

// `pub`, not `pub(crate)`: `daemon::service::review_submit_txn` in the root
// crate builds a review id the same way before handing it to the daemon's
// own transaction path.
#[must_use]
pub fn generate_review_id(task_id: &str) -> String {
