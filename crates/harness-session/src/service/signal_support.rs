...
use crate::types::crosswalk::effective_runtime_session_key;

/// # Errors
/// Returns [`CliError`] when the session state cannot be loaded or an
/// expiry acknowledgment cannot be written or recorded.
pub fn reconcile_expired_pending_signals(
    session_id: &str,
    project_dir: &Path,
) -> Result<(), CliError> {
...
// and `sync_support::signals` read these fields directly to reconcile
// daemon-db state against the file-backed signal directories.
#[derive(Debug, Clone)]
pub struct ExpiredPendingSignal {
    pub agent_id: String,
...
/// # Errors
/// Returns [`CliError`] when the session state cannot be loaded.
pub fn collect_expired_pending_signals(
    session_id: &str,
    project_dir: &Path,
...
/// # Errors
/// Returns [`CliError`] when a signal directory cannot be read.
pub fn collect_expired_pending_signals_for_state(
    state: &SessionState,
    project_dir: &Path,
) -> Result<Vec<ExpiredPendingSignal>, CliError> {
...
/// # Errors
/// Returns [`CliError`] when a signal directory cannot be read.
pub fn collect_expired_pending_signals_for_state_with_context_root_resolver<F>(
    state: &SessionState,
    project_dir: &Path,
...
/// # Errors
/// Returns [`CliError`] when a signal directory cannot be read.
pub fn collect_expired_pending_signals_for_state_in_context_root(
    state: &SessionState,
    context_root: &Path,
) -> Result<Vec<ExpiredPendingSignal>, CliError> {
...
#[must_use]
pub fn signal_context_root(project_dir: &Path) -> PathBuf {
...
pub fn signal_dirs_for_agent_in_context_root(
    runtime: &dyn runtime::AgentRuntime,
...
#[must_use]
pub fn agent_runtime_session_id<'a>(
...
#[must_use]
pub fn runtime_session_matches_agent(
    orchestration_session_id: &str,
...
/// # Errors
/// Returns [`CliError`] when the session state cannot be loaded or a signal
/// directory cannot be read.
pub fn load_signal_record_for_agent(
    session_id: &str,
    agent_id: &str,
    signal_id: &str,
    project_dir: &Path,
...
/// # Errors
/// Returns [`CliError`] when a signal directory cannot be read.
pub fn load_signal_record_for_agent_from_state(
    state: &SessionState,
    agent_id: &str,
    signal_id: &str,
    project_dir: &Path,
...
pub fn apply_signal_ack_result(
...
pub fn apply_task_start_delivery(
    state: &mut SessionState,
    agent_id: &str,
    signal: &Signal,
    now: &str,
...
pub fn expire_task_start_delivery(
    state: &mut SessionState,
    agent_id: &str,
    signal: &Signal,
    now: &str,
...
#[must_use]
pub fn task_id_for_task_start_signal(signal: &Signal) -> Option<&str> {
...
#[must_use]
pub fn signal_is_expired(expires_at: &str) -> bool {
...
/// # Errors
/// Returns [`CliError`] when a signal directory cannot be read.
pub fn signal_records_for_dirs(
    runtime_name: &str,
    agent_id: &str,
    session_id: &str,
