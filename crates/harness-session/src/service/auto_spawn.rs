...
/// `task_board`'s dispatch/evaluation slice reaches this directly across the
/// crate boundary, so it stays `pub` rather than `pub(crate)`.
pub const SPAWN_REVIEWER_COMMAND: &str = "spawn_reviewer";
...
/// be located.
#[must_use]
pub fn maybe_emit_spawn_reviewer(
    state: &SessionState,
    task_id: &str,
    now: &str,
...
mod tests {
    ...
    fn base_state() -> SessionState {
        SessionState {
            ...
            branch_ref: "harness/eadbcb3e-6ef7-53d2-ad56-0347cb7189fc".to_string(),
            title: "t".to_string(),
            context: "c".to_string(),
