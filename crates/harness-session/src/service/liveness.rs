...
#[must_use]
pub fn collect_agent_activity_from_state(
    state: &SessionState,
    session_id: &str,
    project_dir: &Path,
...
pub fn apply_liveness_transitions(
...
pub fn compute_agent_transition(
    ...
    use harness_agents::runtime::liveness::{LivenessStatus, liveness_from_timestamp};
...
pub fn apply_single_transition(
    state: &mut SessionState,
    record: &AgentActivityRecord,
...
pub fn apply_agent_disconnected_with_reason(
    state: &mut SessionState,
    agent_id: &str,
...
pub fn release_agent_tasks(state: &mut SessionState, agent_id: &str, now: &str) {
...
pub fn cleanup_dead_agent_signals(
    activity_map: &[AgentActivityRecord],
    result: &LivenessSyncResult,
    session_id: &str,
    project_dir: &Path,
) {
    use harness_agents::runtime::signal::cleanup_pending_signals;
    let context_root = signal_context_root(project_dir);
