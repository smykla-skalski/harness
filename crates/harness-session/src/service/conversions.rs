...
pub fn detail_to_session_state(detail: &wire::SessionDetail) -> SessionState {
    let agents = detail
        .agents
        .iter()
...
pub fn summary_to_session_state(summary: &wire::SessionSummary) -> SessionState {
    SessionState {
        schema_version: CURRENT_VERSION,
        state_version: 0,
...
/// # Errors
/// Returns [`CliError`] when the daemon request itself fails.
pub fn resolve_runtime_session_via_daemon(
    client: &DaemonClient,
    runtime_name: &str,
...
mod tests {
    use super::*;
    use crate::types::{SessionMetrics, SessionStatus};
