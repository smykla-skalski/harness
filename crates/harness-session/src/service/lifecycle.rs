...
/// # Errors
/// Returns [`CliError`] when the session is not active or `agent_id` is
/// already disconnected.
pub fn apply_leave_session(
...
/// # Errors
/// Returns [`CliError`] when the session is not active.
pub fn apply_update_session_title(
    state: &mut SessionState,
    title: &str,
    now: &str,
) -> Result<(), CliError> {
