...
pub fn apply_submit_for_review(
...
/// mutation, the actor lacks [`SessionAction::UpdateTaskStatus`], the task
/// does not exist, or the task is not `InProgress`.
#[cfg(any(test, feature = "daemon-runtime"))]
pub fn apply_submit_for_review_for_managed_run(
    state: &mut SessionState,
    task_id: &str,
    actor_id: &str,
    summary: Option<&str>,
...
///
/// `pub`, not `pub(crate)`: `daemon::service::review_submit_txn` in the root
/// crate runs this same guard before its own transactional write.
pub fn validate_submit_review(
    state: &SessionState,
    task_id: &str,
...
///
/// `pub`, not `pub(crate)`: `daemon::service::review_submit_txn` in the root
/// crate applies this same mutation inside its own SQL transaction.
pub fn apply_submit_review(
    state: &mut SessionState,
    task_id: &str,
    review: &Review,
    all_reviews: &[Review],
...
mod tests {
    ...
    use crate::types::{Review, ReviewVerdict};
