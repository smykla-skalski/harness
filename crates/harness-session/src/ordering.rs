//! Deterministic ordering for session agents and work items.
//!
//! Every reader that lists a session's roster or task queue expects the same
//! order, so the comparators live with the session types rather than with any
//! one presenter.

use std::cmp::Ordering;

use super::types::{AgentRegistration, AgentStatus, SessionRole, WorkItem};
...
mod tests {
    ...
    use crate::types::{
        SessionRole, TaskQueuePolicy, TaskSeverity, TaskSource, TaskStatus, WorkItem,
    };
