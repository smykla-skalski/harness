...
#[expect(
    ...
    reason = "one linear scenario exercising the index's public surface end to end; splitting it would scatter one coherent story across helpers"
)]
fn index_round_trip_smoke_covers_public_surface() {
    let tmp = tempdir().expect("tempdir");
    #[expect(
        clippy::cognitive_complexity,
        reason = "one linear fixture-setup scenario, not nested branching logic"
    )]
    with_isolated_harness_env(tmp.path(), || {
        ...
        let per_project_sessions = discover_sessions_for(slice::from_ref(&project), true)
