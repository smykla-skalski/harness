...
///
/// # Errors
/// Returns `CliError` when the id is not a safe path segment or fails the
/// session id format check.
pub fn validate_session_id(session_id: &str) -> Result<(), CliError> {
...
///
/// # Errors
/// Returns `CliError` when the id fails the same checks as
/// [`validate_session_id`].
pub fn validate_new_session_id(session_id: &str) -> Result<(), CliError> {
...
pub fn layout_candidates_from_project_dir(
    project_dir: &Path,
    session_id: &str,
...
#[must_use]
pub fn layout_candidates_from_context_root(
    context_root: &Path,
    session_id: &str,
) -> Vec<SessionLayout> {
...
pub fn list_known_session_ids(project_dir: &Path) -> Result<Vec<String>, CliError> {
    let (sessions_root, project_name) = project_layout_parts_from_dir(project_dir)?;
...
pub fn list_known_session_ids_from_context_root(
    context_root: &Path,
