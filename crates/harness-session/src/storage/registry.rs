...
pub use harness_protocol::session_wire::ActiveRegistry;

/// Register a session id in the per-project active-session registry.
///
/// # Errors
/// Returns `CliError` on I/O failures.
pub fn register_active(layout: &SessionLayout) -> Result<(), CliError> {
...
/// # Errors
/// Returns `CliError` on I/O failures.
pub fn deregister_active(layout: &SessionLayout) -> Result<(), CliError> {
    files::validate_session_id(&layout.session_id)?;
    files::with_lock(layout, "active-registry", || {
        let path = files::active_registry_path(layout);
        let mut registry = load_registry_at(&path);
...
pub fn load_active_registry_for(project_dir: &Path) -> Result<ActiveRegistry, CliError> {
...
#[must_use]
pub fn load_active_registry_for_context_root(context_root: &Path) -> ActiveRegistry {
...
pub use harness_protocol::session_wire::ProjectOriginRecord;
...
pub fn record_project_origin(project_dir: &Path) -> Result<(), CliError> {
...
/// # Errors
/// Returns `CliError` on I/O failures.
// `pub` rather than `pub(crate)`: the root crate's daemon adoption path
// calls this across the crate boundary now, unconditionally (the old
// `daemon-runtime` cfg gate was root's own feature, not one this crate
// carries).
pub fn record_adopted_session_root(
    project_dir: &Path,
    session_id: &str,
    session_root: &Path,
) -> Result<(), CliError> {
...
#[must_use]
pub fn load_project_origin(context_root: &Path) -> Option<ProjectOriginRecord> {
    let path = context_root.join(PROJECT_ORIGIN_FILE);
