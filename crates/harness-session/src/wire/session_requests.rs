...
// Lives beside the wire request that carries it rather than with the rest
// of improver apply/preview logic in the root crate's `session::service`:
// that domain stays in the root crate as a separate, larger extraction, and
// this leaf enum is the only piece of it the wire layer needs. Root's
// `session::service::improver_state` re-exports this exact path so every
