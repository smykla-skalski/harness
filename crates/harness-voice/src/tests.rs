...
fn write_stale_session(base_dir: &Path, voice_session_id: &str) {
    fs::create_dir_all(session_dir(base_dir, voice_session_id)).expect("create voice session dir");
    fs::write(
        session_record_path(base_dir, voice_session_id),
    ...
    fs::write(chunks_path(base_dir, voice_session_id), [1_u8, 2, 3, 4]).expect("write chunks");
    fs::write(chunks_metadata_path(base_dir, voice_session_id), "{}\n")
        .expect("write chunk metadata");
    fs::write(transcript_path(base_dir, voice_session_id), "{}\n").expect("write transcript");
}
...
fn start_session_cleans_up_stale_voice_sessions() {
    ...
    write_stale_session(base_dir, stale_session_id);

    start_session(base_dir, "session-b", &request()).expect("start");

    assert!(
        !session_dir(base_dir, stale_session_id).exists(),
...
fn append_audio_chunk_error_cleans_up_voice_session() {
    ...
    runtime.block_on(async {
        let started = start_session(base_dir, "session-a", &request()).expect("start");
        let dir = session_dir(base_dir, &started.voice_session_id);

        let error = append_audio_chunk(base_dir, &started.voice_session_id, &chunk(2))
            .await
...
fn voice_session_persists_chunk_sequence_and_cleans_up() {
    ...
    runtime.block_on(async {
        let started = start_session(base_dir, "session-a", &request()).expect("start");
        append_audio_chunk(base_dir, &started.voice_session_id, &chunk(1))
            .await
            .expect("chunk");
        append_transcript(
            base_dir,
