...
See [variant-detection.md](variant-detection.md) for the full methodology, signal strength classification, and a worked MOTB example.
...
- OTel collector ConfigMaps must use the `debug` exporter, not `logging`. The `logging` exporter was removed in recent collector versions and causes CrashLoopBackOff at startup.

### Verification workflow

For each manifest generated for the suite, run all 8 checks:
...
7. Confirm spec nesting matches the policy pattern: `from[].default` for from-based, `to[].default` for to-based, `rules[].default` for rules-based. Use the matching nesting level, not `spec.default`, on from/to policies
8. If unsure about a field, read the CRD file (`deployments/charts/kuma/crds/kuma.io_<plural>.yaml`) to verify

If any check fails, fix the manifest and re-run all 8 checks. Repeat until the manifest passes before including it in the suite.
...
- When the code references delegated gateways (`IsDelegatedGateway()`, `gateway.type: DELEGATED`), the test workload should be Kong Gateway - not nginx or a generic proxy. See [suite-structure.md](suite-structure.md) (Domain knowledge) for setup details.
