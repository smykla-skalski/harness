...
10. [Command authoring defaults](#command-authoring-defaults)
...
Not all groups apply to every feature. Skip groups that don't make sense, but document why in the suite metadata so the next author or runner can tell the difference between intentional scope and missing coverage.
