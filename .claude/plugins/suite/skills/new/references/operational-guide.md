...
Interactive step-by-step suite generation. Uses the same authoring state, workers, approval gates, and final report as generate mode. The only differences are:
...
| KSA006 | worker contract | Workers must save structured payloads through `harness authoring save` |
...
- If `harness authoring begin` was not run after deriving `SUITE_DIR`, stop and run `harness authoring begin --skill suite:new --repo-root "${REPO_ROOT}" --feature "${FEATURE}" --mode interactive|bypass --suite-dir "${SUITE_DIR}" --suite-name "${SUITE_NAME}"` before launching workers because the compact worker state must exist before caching results.
- If `harness authoring approval-begin` was not run after deriving `SUITE_DIR`, stop and run `harness authoring approval-begin --skill suite:new --mode interactive|bypass --suite-dir "${SUITE_DIR}"` before the canonical approval gates because the hooks fail closed on missing approval state.
