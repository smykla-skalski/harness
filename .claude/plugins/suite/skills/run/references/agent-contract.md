...
14. **The test groups table is the execution plan.** Every group listed in the suite's test groups table must be executed. Do not skip groups because they need a different cluster profile than what's currently running. If a group requires multi-zone, tear down single-zone and bring up multi-zone. If that's impractical, ask the user via AskUserQuestion. Only the `skipped groups` field in suite metadata lists groups that are excluded.
