...
fn make_state(phase: CreatePhase, mode: ApprovalMode) -> CreateWorkflowState {
    CreateWorkflowState {
        mode,
        phase,
...
fn create_phase_display() {
    let cases = [
        (CreatePhase::Discovery, "discovery"),
        (CreatePhase::PrewriteReview, "prewrite_review"),
        (CreatePhase::Writing, "writing"),
        (CreatePhase::PostwriteReview, "postwrite_review"),
        (CreatePhase::Complete, "complete"),
        (CreatePhase::Cancelled, "cancelled"),
    ];
...
#[test]
fn create_answer_serialization() {
    let cases = [
        (CreateAnswer::ApproveProposal, "Approve proposal"),
        (CreateAnswer::RequestChanges, "Request changes"),
        (CreateAnswer::Cancel, "Cancel"),
        (CreateAnswer::ApproveSuite, "Approve suite"),
        (CreateAnswer::CopyCommand, "Copy command"),
        (CreateAnswer::Skip, "Skip"),
    ];
    for (variant, expected) in cases {
        let json = serde_json::to_value(variant).unwrap();
        assert_eq!(json, expected);
...
fn full_state_serialization_round_trip() {
    ...
    let loaded: CreateWorkflowState = serde_json::from_value(json).unwrap();
...
fn can_write_discovery_denies() {
    let state = make_state(CreatePhase::Discovery, ApprovalMode::Interactive);
...
fn can_write_postwrite_review_denies() {
    let state = make_state(CreatePhase::PostwriteReview, ApprovalMode::Interactive);
...
#[test]
fn next_action_none() {
    assert_eq!(next_action(None), CreateNextAction::ReloadState);
...
fn next_action_bypass() {
    let state = make_state(CreatePhase::Discovery, ApprovalMode::Bypass);
    assert_eq!(next_action(Some(&state)), CreateNextAction::ContinueBypass);
...
fn next_action_each_phase() {
    let phases = [
        (CreatePhase::Discovery, "discovery"),
        (CreatePhase::PrewriteReview, "pre-write"),
        (CreatePhase::PostwriteReview, "post-write"),
        (CreatePhase::Cancelled, "cancelled"),
        (CreatePhase::Complete, "approved"),
    ];
...
fn next_action_writing_with_suite_written() {
    let mut state = make_state(CreatePhase::Writing, ApprovalMode::Interactive);
    state.draft.suite_tree_written = true;
    assert_eq!(next_action(Some(&state)), CreateNextAction::ApplyEditRound);
...
#[test]
fn suite_create_path_allowed_suite_md() {
    let suite = Path::new("/tmp/suite");
    assert!(suite_create_path_allowed(&suite.join("suite.md"), suite));
}

#[test]
fn suite_create_path_allowed_groups() {
    let suite = Path::new("/tmp/suite");
    assert!(suite_create_path_allowed(
...
#[test]
fn suite_create_path_allowed_baseline() {
    let suite = Path::new("/tmp/suite");
    assert!(suite_create_path_allowed(
...
#[test]
fn suite_create_path_denied_outside() {
    let suite = Path::new("/tmp/suite");
    assert!(!suite_create_path_allowed(
...
#[test]
fn suite_create_path_denied_random_file_in_suite() {
    let suite = Path::new("/tmp/suite");
    assert!(!suite_create_path_allowed(&suite.join("random.txt"), suite));
}
...
fn session_info_suite_path() {
    ...
    let info_none = CreateSessionInfo {
        repo_root: None,
        feature: None,
        suite_name: None,
        suite_dir: None,
...
fn deserialize_rejects_legacy_flat_shape() {
    ...
    let error = serde_json::from_value::<CreateWorkflowState>(legacy).unwrap_err();
