...
use crate::session::types::{
    Review, ReviewClaim, ReviewConsensus, ReviewPoint, ReviewPointState, ReviewVerdict,
    ReviewerEntry, WorkItem,
};
...
/// Worker-side response to a consensus that requested changes.
///
/// Each point in `agreed` is marked `Resolved` and will be addressed by
/// the worker; each point in `disputed` is marked `Disputed` and goes
/// back to the reviewers for a fresh round. Incrementing `review_round`
/// gates the arbitration trigger. When all consensus points are agreed
/// the task returns to `InProgress` with the worker reassigned so the
/// queue can schedule the rework. When any point is disputed the task
/// stays `InReview`, the prior consensus is cleared, and each reviewer
/// entry's `submitted_at` is reset so the next round can form.
///
/// # Errors
/// - session is not active
/// - actor lacks [`SessionAction::RespondReview`]
/// - task missing, not `InReview`, or no consensus to respond to
/// - actor is not the original awaiting-review submitter
