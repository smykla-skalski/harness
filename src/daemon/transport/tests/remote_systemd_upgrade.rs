...
use super::super::remote_systemd_upgrade_lifecycle::{
    LockedLifecycle, RemoteSystemdArtifact, RemoteSystemdRecoveryOutcome,
    RemoteSystemdUpgradeOutcome, acquire_with_trusted_controller,
...
fn same_digest_legacy_unit_uses_transaction_and_rolls_back_to_simple() {
    ...
    assert_eq!(rolled_back.outcome, RemoteSystemdUpgradeOutcome::RolledBack);
    assert!(
        fs::read_to_string(&fixture.unit)
...
fn failed_upgrade_restores_binary_database_and_all_harness_state() {
    ...
    let evidence_metadata = fs::symlink_metadata(&failed_state).expect("evidence metadata");
    assert_eq!(evidence_metadata.mode() & 0o777, 0o700);
    assert_eq!(evidence_metadata.uid(), uzers::get_current_uid());
    assert_eq!(evidence_metadata.gid(), uzers::get_current_gid());
    assert_eq!(
...
fn explicit_rollback_restores_retained_database_generation_and_is_reversible() {
    ...
    assert!(fixture.operation.store_path.join("previous").is_dir());
    assert!(
        installed_is_candidate(&fixture.operation.store_path.join("recovery-controller")),
        "rollback must persist the trusted candidate coordinator before restoring the old binary"
    );
