...
fn retry_finishes_persistently_inhibited_uninstall_after_reload_failure() {
    ...
    let runner = |args: &[String]| {
        let output = match command(args) {
            Some("show") if fixture.unit_path.exists() => {
            ...
            Some("show") if !removal_reloaded.get() => {
            ...
            Some("daemon-reload") if !fixture.unit_path.exists() && fail_reload.replace(false) => {
                command_output(5, "", "injected reload failure")
            }
            Some("daemon-reload") => {
...
fn absent_files_with_hidden_vendor_unit_remain_inhibited() {
    let fixture = UninstallFixture::new();
    std::fs::remove_file(&fixture.env_path).expect("remove environment");
    std::fs::remove_file(&fixture.unit_path).expect("remove unit");
