...
fn recovery_units_repeat_and_treat_the_operation_lock_as_deferred_success() {
    ...
    assert!(service.contains("recover-systemd --store-path"));
...
fn assert_partial_arm_cleaned(fixture: &UpgradeFixture, runner: &ScriptedSystemd<'_>) {
    assert!(!fixture.operation.store_path.join("armed.json").exists());
    assert!(
        !fixture
            .operation
        ...
        let _ = upgrade_remote_systemd_with(
            ...
            &|plan, expected, run| {
                if installed_is_candidate(&fixture.binary) {
                    panic!("simulated coordinator SIGKILL after candidate start");
                }
...
fn rollback_evidence_is_moved_after_terminal_phase_and_before_old_restart() {
    ...
    assert!(!fixture.operation.store_path.join("pending").exists());
    assert!(
        fs::read_dir(&fixture.operation.store_path)
