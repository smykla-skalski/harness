...
fn untrusted_candidate_coordinator_is_rejected_before_store_mutation() {
    ...
    assert!(error.to_string().contains("coordinator digest"));
    assert!(error.to_string().contains("--candidate-path"));
    assert!(!fixture.operation.store_path.exists());
...
fn controller_is_reverified_after_lifecycle_lock_acquisition() {
    let fixture = UpgradeFixture::new();
    let controller = fixture.binary.with_file_name("running-harness");
    let replacement = "#!/bin/sh\necho 'harness replacement'\n";
    fs::copy(&fixture.binary, &controller).expect("snapshot running controller binary");
    let mut operation = fixture.operation.clone();
    operation.controller_path = controller;

    let error = acquire_with_trusted_controller(&operation, || {
        fs::write(&fixture.binary, replacement)
            .expect("replace installed binary during lifecycle lock acquisition");
        Ok(())
    })
    .expect_err("changed binary must invalidate the waiting coordinator");
