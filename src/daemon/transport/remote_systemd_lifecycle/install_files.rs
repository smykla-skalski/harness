...
fn secure_existing_unit(
    ...
    if existing != expected {
        return Err(CliErrorKind::workflow_parse(format!(
            "existing unit {unit} differs from the requested installation; keep it active and use upgrade-systemd so binary, unit, and database state are rollback-protected"
        ))
        .into());
    }
    apply_trusted_metadata(&file, path, mode)
