...
#[tokio::test]
async fn get_managed_agents_marks_orphaned_running_terminal_snapshots_inactive() {
    let state = test_http_state_with_async_db_only().await;
    let async_db = state.async_db.get().expect("async db").clone();
    async_db
        .save_agent_tui(&AgentTuiSnapshot {
            ...
            signal: None,
            error: None,
            created_at: "2026-04-30T17:00:00Z".into(),
            updated_at: "2026-04-30T17:01:00Z".into(),
        })
        .await
    ...
    assert_eq!(agents.len(), 1);
    assert_eq!(agents[0]["kind"].as_str(), Some("terminal"));
    assert_eq!(agents[0]["snapshot"]["status"].as_str(), Some("exited"));
    assert_eq!(
        agents[0]["snapshot"]["error"].as_str(),
        Some("managed terminal agent process missing from daemon runtime")
    );

    let persisted = async_db
        .agent_tui("agent-tui-orphaned")
        .await
