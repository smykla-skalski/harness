...
//! types. Routes are collected via `utoipa-axum`, so registering a handler on
//! the daemon router also registers its `OpenAPI` path; `x-websocket-method`
//! extensions and generator provenance are injected at the JSON layer from the
//! authoritative [`HTTP_API_CONTRACT`].
//!
//! The checked-in artifact `docs/api/openapi.json` is regenerated by
//! `tools/openapi-codegen` (`mise run openapi:generate`) and drift-checked by
...
/// Assemble the daemon router with every domain's routes. Registering a handler
/// via `utoipa-axum` contributes both its axum route and its `OpenAPI` path, so
/// this is the same router the daemon serves - the schema cannot drift from the
/// routes. `HarnessDaemonApi` seeds the document `info` and shared components.
pub(super) fn daemon_openapi_router() -> OpenApiRouter<super::DaemonHttpState> {
    OpenApiRouter::with_openapi(HarnessDaemonApi::openapi())
        .merge(super::core::core_routes())
        .merge(super::sessions::session_routes())
        .merge(super::reviews::reviews_routes())
        .merge(super::task_board::task_board_routes())
        .merge(super::tasks::task_routes())
        .merge(super::improver::improver_routes())
        .merge(super::agents::agent_routes())
        .merge(super::managed_agents::managed_agent_routes())
