...
fn write_manifest_serializes_concurrent_writers_before_loading_next_revision() {
    ...
    with_isolated_harness_env(tmp.path(), || {
        ...
        let second_hook =
            second_before_release.or_else(|| event_rx.recv_timeout(Duration::from_secs(2)).ok());
...
fn write_manifest_sets_updated_at_to_non_empty_string() {
    let tmp = tempdir().expect("tempdir");
    with_isolated_harness_env(tmp.path(), || {
        let written = write_manifest(&sample_manifest(555, "http://127.0.0.1:0")).expect("write");
        assert!(
