...
/// agent_session_id)` directly and stays strict on explicit
/// `runtime_session_id` bindings. Restricted to active sessions and alive
/// agents (`status IN ('active', 'idle')`) so dead rows do not pollute the
/// resolution.
