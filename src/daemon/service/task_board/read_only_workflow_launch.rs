...
use crate::task_board::{
    AgentMode, DispatchAppliedTask, TaskBoardItem, TaskBoardPullRequestIdentity,
    TaskBoardReadOnlyWorkflowLaunch, TaskBoardWorkflowKind,
    resolve_task_board_pull_request_identity, resolve_task_board_reviewers,
...
pub(super) async fn prepare_read_only_workflow_launch(
    ...
    let item = db.task_board_item_snapshot(item_id).await?.item;
    if !is_read_only_workflow(item.workflow_kind) {
        return Ok(None);
    }
