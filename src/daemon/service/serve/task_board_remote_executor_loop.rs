...
pub(super) fn spawn_task_board_remote_executor_loop(
    ...
    tokio::spawn(async move {
        loop {
            ...
            if let Err(error) = Box::pin(reconcile_remote_executor_assignments(
                &state,
                Some(&shutdown_rx),
            ))
            .await
...
async fn reconcile_remote_executor_assignments(
    ...
    for assignment_id in assignment_ids {
        if let Err(error) = Box::pin(reconcile_remote_executor_assignment_with_shutdown(
            state,
            db.as_ref(),
            &assignment_id,
            shutdown_rx,
...
pub(crate) async fn reconcile_task_board_remote_executor_tick(
    ...
    for assignment_id in executor_assignment_ids(db.as_ref()).await? {
...
async fn reconcile_active_remote_worker(
    ...
    Box::pin(execute_and_reconcile_remote_worker(
        state,
        db,
        record,
        &offer,
...
async fn prepare_active_remote_worker(
    ...
    // Durable permit/run evidence recovers without provisioning or another Start.
