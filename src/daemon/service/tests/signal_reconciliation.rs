...
fn session_detail_core_db_direct_reopens_expired_pending_delivery() {
    with_temp_project(|project| {
        ...
        let indexed_signal = db
            ...
            .into_iter()
            .find(|candidate| {
                candidate.agent_id == worker_id
                    && candidate.signal.signal_id == signal.signal.signal_id
            })
        ...
        let reconciled_task = reconciled
            .tasks
            .get(&task_id)
            .expect("reconciled task present");
