...
fn write_active_policy_graph(root: &PathBuf, graph: PolicyGraph) {
...
fn approve_wait_merge_policy_graph() -> PolicyGraph {
    workflow_graph(vec![
...
fn merge_only_policy_graph() -> PolicyGraph {
...
struct TestReviewsPolicyExecutor {
    recorded_actions: Arc<Mutex<Vec<String>>>,
}
