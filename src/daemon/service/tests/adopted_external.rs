// This lived beside `session::index`'s own tests until the crate split:
// `harness-session`'s own test binary can reach `harness::daemon` only as a
// dev-only dependency and gets a second, distinct compilation of
// `harness-session` for it, so `AdoptionOutcome` there and `AdoptionOutcome`
// as `daemon::service::adopt_session_record` expects are two different
// types despite the identical source. Living here, in the root crate's own
// test suite, avoids the duplicate-compilation edge entirely.
use super::*;
...
fn discover_sessions_finds_adopted_external_session_root() {
    ...
    with_isolated_harness_env(tmp.path(), || {
        ...
        let resolved = index::resolve_session(session_id).expect("resolve adopted session");
