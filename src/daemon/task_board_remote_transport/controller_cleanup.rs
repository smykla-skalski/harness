...
impl RemoteExecutionControllerClient {
    pub(crate) async fn observe_cleanup(
        ...
        (RemoteCleanupObservationResponse, TaskBoardRemoteMutationOutcome),
        RemoteExecutionControllerError,
    > {
        let trust = self
            .current_configured_host_trust_for_lifecycle(db)
            .await?;
        if let Some(response) = db
            .claim_task_board_remote_cleanup_observation_fenced(
                request,
                &self.host_id,
                &trust,
            )
        ...
        let response = self.client.observe_cleanup(request).await?;
        let outcome = db
            .record_task_board_remote_cleanup_observation(
                request,
                &response,
                &self.host_id,
                &trust,
