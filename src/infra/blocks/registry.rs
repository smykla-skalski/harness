...
///
/// The suite workflow once declared these as preflight requirements
/// (`BlockRequirement::Docker`, `::Kubernetes`, `::Helm`, `::Kuma`, and
/// friends). That workflow is retired, and matching or parsing those
/// requirement names has no remaining reader. Only this union of binary
/// names survives: the ACP protocol reads it through `all_denied_binaries`
/// below as the set an agent may not create.
const DENIED_BINARIES: &[&str] = &["docker", "helm", "k3d", "kubectl", "kubectl-validate", "kumactl"];
