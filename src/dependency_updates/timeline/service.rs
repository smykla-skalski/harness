...
use super::{
    DependencyUpdatesTimelineRequest, DependencyUpdatesTimelineResponse, TimelinePageInfo,
};
...
pub(crate) async fn fetch_timeline_page<C: TimelineClient>(
    ...
    client: &C,
    now: Instant,
) -> Result<DependencyUpdatesTimelineResponse, TimelineError> {
    ...
    let mut entries: Vec<DependencyUpdateTimelineEntry> = Vec::with_capacity(nodes.len());
    for node in &nodes {
        let typename = node
            .get("__typename")
            .and_then(Value::as_str)
...
fn push_with_review_truncation(
    entries: &mut Vec<DependencyUpdateTimelineEntry>,
    mut entry: DependencyUpdateTimelineEntry,
    truncated: bool,
) {
    if truncated {
        if let DependencyUpdateTimelineEntry::Review(ref mut r) = entry {
...
fn push_with_thread_truncation(
    ...
    if truncated {
        if let DependencyUpdateTimelineEntry::ReviewThread(ref mut t) = entry {
