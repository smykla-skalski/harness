...
fn request_for(pr: &str) -> DependencyUpdatesTimelineRequest {
    DependencyUpdatesTimelineRequest {
...
async fn review_thread_with_paginated_comments_is_fully_drained() {
    ...
    let DependencyUpdateTimelineEntry::ReviewThread(t) = &resp.entries[0] else {
