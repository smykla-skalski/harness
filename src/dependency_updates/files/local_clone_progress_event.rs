...
/// inside the event name itself; the `dependency_updates.` prefix here
/// is part of the event identifier so subscribers can filter by prefix).
pub const DEPENDENCY_UPDATES_LOCAL_CLONE_PROGRESS_EVENT: &str =
    "dependency_updates_local_clone_progress";
        ...
        match build_stream_event(&payload) {
            ...
            Err(error) => {
                warn!(
                    ...
                    event = DEPENDENCY_UPDATES_LOCAL_CLONE_PROGRESS_EVENT,
...
fn build_stream_event(
    ...
    Ok(StreamEvent {
        event: DEPENDENCY_UPDATES_LOCAL_CLONE_PROGRESS_EVENT.to_string(),
...
mod tests {
    ...
    fn broadcast_sink_pushes_payload_into_stream_event() {
        ...
        assert_eq!(event.event, DEPENDENCY_UPDATES_LOCAL_CLONE_PROGRESS_EVENT);
    ...
    fn event_name_constant_matches_dotted_prefix_convention() {
        ...
        // can filter by "dependency_updates_" prefix without parsing.
        assert!(
            DEPENDENCY_UPDATES_LOCAL_CLONE_PROGRESS_EVENT
                .starts_with("dependency_updates_local_clone_progress")
        );
    }
}
