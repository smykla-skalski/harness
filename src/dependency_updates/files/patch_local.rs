...
//! `DependencyUpdateFilePatch` rows with full content (never truncated)
...
/// deletions == 0.
#[must_use]
pub fn parse_git_diff(raw: &str) -> Vec<DependencyUpdateFilePatch> {
    let mut out = Vec::new();
    let mut current: Option<DependencyUpdateFilePatch> = None;
    ...
    for line in raw.split_inclusive('\n') {
        ...
        if line.starts_with("Binary files ") {
            patch.status = DependencyUpdateFileChangeType::Modified;
            current_body.push_str(line);
            continue;
        }
        if line.starts_with("new file mode ") {
            patch.status = DependencyUpdateFileChangeType::Added;
            current_body.push_str(line);
            continue;
        }
        if line.starts_with("deleted file mode ") {
            patch.status = DependencyUpdateFileChangeType::Deleted;
            current_body.push_str(line);
            continue;
        }
        if line.starts_with("rename from ") || line.starts_with("rename to ") {
            patch.status = DependencyUpdateFileChangeType::Renamed;
            current_body.push_str(line);
            continue;
        }
        if line.starts_with("copy from ") || line.starts_with("copy to ") {
            patch.status = DependencyUpdateFileChangeType::Copied;
            current_body.push_str(line);
            continue;
        }
        if line.starts_with("@@ ") {
...
mod tests {
    ...
    fn parse_simple_two_file_diff() {
        ...
        assert_eq!(patches[0].status, DependencyUpdateFileChangeType::Modified);
    ...
    fn parse_renamed_file_diff() {
        ...
        assert_eq!(patches[0].status, DependencyUpdateFileChangeType::Renamed);
    ...
    fn parse_deleted_file_diff() {
        ...
        assert_eq!(patches[0].status, DependencyUpdateFileChangeType::Deleted);
        assert_eq!(patches[0].additions, 0);
    ...
    fn served_by_is_local_clone_for_all_outputs() {
        ...
        for p in parse_git_diff(raw) {
            assert_eq!(p.served_by, DependencyUpdateFileServedBy::LocalClone);
        }
    }
    ...
    fn parse_copy_diff_sets_copied_status() {
        ...
        let patches = parse_git_diff(raw);
        assert_eq!(patches.len(), 1);
        assert_eq!(patches[0].status, DependencyUpdateFileChangeType::Copied);
    }
