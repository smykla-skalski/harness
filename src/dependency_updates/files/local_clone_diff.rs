...
//! [`DependencyUpdateFilePatch`] rows with hunk text already formatted in
...
fn patch_for_change(
    ...
    fetched_at: &str,
    head_ref_oid: &str,
) -> Result<Option<DependencyUpdateFilePatch>, LocalCloneRuntimeError> {
    ...
    Ok(Some(DependencyUpdateFilePatch {
        path,
        patch: patch_text,
        status,
        additions,
...
mod tests {
    use super::*;
    use crate::dependency_updates::files::local_clone::LocalCloneRoot;
    ...
    async fn compute_patches_emits_added_modified_deleted_rows() {
        ...
        for patch in by_path.values() {
            assert_eq!(patch.served_by, DependencyUpdateFileServedBy::LocalClone);
