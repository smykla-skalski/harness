...
pub async fn session_start(
    ...
    task::spawn_blocking(move || {
        ...
        let session_id = resolve_or_create_session_id(agent, &project_dir, session_id_hint.as_deref())?;
...
pub async fn session_stop(
    ...
    task::spawn_blocking(move || {
        let session_id = session_id_hint
            .or_else(|| storage::current_session_id(&project_dir, agent).ok().flatten())
    ...
    RUNTIME
        .block_on(async move {
            task::spawn_blocking(move || {
                ...
                if matches!(context.event, crate::hooks::protocol::context::NormalizedEvent::SessionStart)
                {
                    storage::set_current_session_id(&project_dir, agent, &session_id)?;
                }
                ...
                if matches!(context.event, crate::hooks::protocol::context::NormalizedEvent::SessionEnd)
                {
                    storage::clear_current_session_id(&project_dir, agent)?;
                }
                Ok(())
...
pub fn project_dir_for_context(context: &NormalizedHookContext) -> Result<PathBuf, CliError> {
    context
        ...
        .map_or_else(
            || Err(CliErrorKind::workflow_io("missing project directory for agent event".to_string()).into()),
            Ok,
        )
}
...
fn default_session_id(agent: HookAgent) -> String {
    format!(
        ...
        crate::workspace::utc_now().replace([':', '-'], "")
    )
}
