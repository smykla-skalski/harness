...
pub(super) fn render_local_skills(
    ...
    let entry = outputs
        ...
        .or_insert_with(|| PlannedOutput {
            managed_root: claude_skills_root.clone(),
            files: BTreeMap::new(),
            symlinks: BTreeMap::new(),
        });

    let read_dir = std::fs::read_dir(&source_root).map_err(|error| io_err(&error))?;
    for dir_entry in read_dir {
        let dir_entry = dir_entry.map_err(|error| io_err(&error))?;
        ...
        if !dir_entry.file_type().map_err(|e| io_err(&e))?.is_dir() {
            continue;
        }
        let link = claude_skills_root.join(name_str);
        // Relative target: from .claude/skills/<name> up two levels to repo root,
        // then into local-skills/claude/<name>/.
        let target = PathBuf::from("../../local-skills/claude").join(name_str);
        entry.symlinks.insert(link, target);
    }
...
mod tests {
    ...
    fn plans_symlinks_for_each_subdir() {
        ...
        let output = outputs.get(&claude_skills).expect("output for .claude/skills");
    ...
    fn skips_plain_files_in_source_root() {
        ...
        assert!(output.symlinks.contains_key(&claude_skills.join("real-skill")));
    }
}
