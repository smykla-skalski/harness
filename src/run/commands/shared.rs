...
pub(crate) fn resolve_run_services(args: &RunDirArgs) -> Result<RunServices, CliError> {
    resolve_run_services_with_blocks(args, Arc::new(BlockRegistry::production()))
}

/// Resolve a run aggregate and build services with the provided adapters.
///
/// # Errors
/// Returns `CliError` when the run aggregate cannot be loaded.
pub(crate) fn resolve_run_services_with_blocks(
    args: &RunDirArgs,
    blocks: Arc<BlockRegistry>,
) -> Result<RunServices, CliError> {
    let aggregate = resolve_run_aggregate(args)?;
    RunServices::from_context_with_blocks(aggregate, blocks)
}
