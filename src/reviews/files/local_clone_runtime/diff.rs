...
fn change_status(change: &Change<'_, '_, '_>) -> ReviewFileChangeType {
    match change {
        Change::Addition { .. } => ReviewFileChangeType::Added,
        Change::Deletion { .. } => ReviewFileChangeType::Deleted,
        Change::Modification { .. } => ReviewFileChangeType::Modified,
        Change::Rewrite { copy, .. } => {
            if *copy {
...
fn stats_for(patches: &[ReviewFilePatch]) -> LocalCloneDiffStats {
    LocalCloneDiffStats {
...
fn local_tracking_ref_for(remote_ref: &str) -> String {
    ...
    let suffix = suffix
        ...
        .collect::<String>();
    format!("refs/harness/reviews/{suffix}")
}
