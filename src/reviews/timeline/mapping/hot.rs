...
pub(super) fn map_issue_comment(node: &Value) -> Option<ReviewTimelineEntry> {
...
pub(super) fn map_pull_request_review(node: &Value) -> Option<ReviewTimelineEntry> {
    ...
    Some(ReviewTimelineEntry::Review(ReviewEntry {
        id,
        created_at,
        actor,
        state,
...
pub(super) fn map_pull_request_commit(node: &Value) -> Option<ReviewTimelineEntry> {
    ...
    Some(ReviewTimelineEntry::Commit(CommitEntry {
        id,
        created_at,
        actor,
        oid,
...
pub(super) fn map_head_ref_force_pushed(node: &Value) -> Option<ReviewTimelineEntry> {
    let id = parse_string_required(node, "id")?;
    let created_at = parse_iso8601(node.get("createdAt"))?;
    ...
    let ref_name = node
        ...
        .and_then(Value::as_str)
        .map(str::to_string);
    Some(ReviewTimelineEntry::HeadRefForcePushed(
