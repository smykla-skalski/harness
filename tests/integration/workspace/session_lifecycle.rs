...
const LIFECYCLE_SESSION_ID: &str = "00000000-0000-4000-8000-000000000401";
const LIST_SESSION_ID: &str = "00000000-0000-4000-8000-000000000402";
const STATUS_SESSION_ID: &str = "00000000-0000-4000-8000-000000000403";
const DELETE_SESSION_ID: &str = "00000000-0000-4000-8000-000000000404";
const BASE_REF_SESSION_ID: &str = "00000000-0000-4000-8000-000000000405";
...
fn session_start_creates_workspace_layout() {
    ...
    let state = start_session_via_http(&home, &xdg, &project, LIFECYCLE_SESSION_ID);

    assert_eq!(
        state.session_id, LIFECYCLE_SESSION_ID,
    ...
    assert_eq!(state.branch_ref, format!("harness/{LIFECYCLE_SESSION_ID}"));
    ...
    assert!(
        active_text.contains(LIFECYCLE_SESSION_ID),
...
fn session_list_shows_session_id() {
    ...
    assert_eq!(
        state.branch_ref,
        format!("harness/{LIST_SESSION_ID}"),
    ...
    assert!(
        list_text.contains(LIST_SESSION_ID),
...
fn session_status_shows_session_id_and_title() {
    ...
    start_session_via_http(&home, &xdg, &project, STATUS_SESSION_ID);
    ...
    assert!(
        status_text.contains(STATUS_SESSION_ID),
...
fn session_delete_removes_worktree_branch_and_state_files() {
    ...
    // forward them.
    let state = start_session_via_http(&home, &xdg, &project, DELETE_SESSION_ID);
    ...
    let http_status = delete_session_via_http(&home, &xdg, DELETE_SESSION_ID);
    ...
    assert!(
        !branches
            .iter()
            .any(|branch| branch == &format!("harness/{DELETE_SESSION_ID}")),
        "branch harness/{DELETE_SESSION_ID} must be deleted; found: {branches:?}"
    );
    ...
    if layout.active_registry().exists() {
        ...
        assert!(
            !active_text.contains(DELETE_SESSION_ID),
...
fn session_start_with_base_ref_routes_to_requested_branch() {
    ...
    let state = start_session_with_base_ref(&home, &xdg, &project, BASE_REF_SESSION_ID, "release");
