...
#[test]
fn prepare_app_entitlements_runs_as_scheme_pre_action() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR"));
    let build_phases = read_repo_file(
        root,
        "apps/harness-monitor-macos/Tuist/ProjectDescriptionHelpers/BuildPhases.swift",
    ...
    assert!(
        phase.contains("script: \"prepare-app-entitlements.sh\"")
            && phase.contains("target: .target(\"HarnessMonitor\")"),
        "app entitlements must be generated by a scheme pre-action before Xcode constructs target packaging tasks"
    );
    assert!(
        project.contains("$(PROJECT_TEMP_DIR)/GeneratedAppEntitlements/$(TARGET_NAME).codesign.entitlements"),
        "CODE_SIGN_ENTITLEMENTS should use a project-temp file so Xcode does not scan a source app-group entitlement"
    );
    assert!(
        !build_phases.contains("public static func prepareAppEntitlements(variant: ProvenanceVariant) -> TargetScript"),
        "app entitlements must not be generated by a target script phase because ProcessProductPackaging consumes CODE_SIGN_ENTITLEMENTS before target phases"
    );
}
...
fn previewable_and_app_targets_package_monitor_asset_catalog() {
    ...
    assert!(
        previewable.contains(".debug(name: \"Debug\", settings: BuildSettings.canvasPreviewCompilationOverrides)"),
        "HarnessMonitorUIPreviewable Debug builds must disable SDK prefix mapping so active-scheme Canvas preview thunks can load SwiftData/SwiftUI cross-import overlays"
    );
...
fn monitor_targets_disable_xcode_managed_app_group_registration() {
    ...
    assert!(
        project.contains("BuildPhases.prepareAppEntitlementsPreAction()"),
        "schemes that build HarnessMonitor should generate full local app-group entitlements before code signing"
    );
