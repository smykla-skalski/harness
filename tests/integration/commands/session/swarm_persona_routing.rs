...
fn persona_matching_worker_picks_up_reassignable_task_over_bare_agent() {
    ...
    with_session_test_env(tmp.path(), "integ-persona-routing", || {
        ...
        let session_id = session_uuid("persona-rt");
        ...
        .unwrap();
        service::assign_task(&session_id, &first.task_id, &bare_id, &leader_id, &project).unwrap();
        service::update_task(
            &session_id,
            &first.task_id,
        ...
        let layout =
            harness::session::storage::layout_from_project_dir(&project, &session_id).unwrap();
        let state_before = service::session_status(&session_id, &project).unwrap();
