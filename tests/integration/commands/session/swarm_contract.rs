...
use super::{session_cmd, session_uuid, with_session_test_env};
...
fn leader_request_with_fallback_stays_non_leader_until_transfer() {
    ...
    with_session_test_env(tmp.path(), "swarm-fallback", || {
        ...
        let session_id = session_uuid("swarm-fallback-1");
        ...
        let joined = service::session_status(&session_id, &project).expect("status");
        let improver = joined
            .agents
            .values()
...
fn observer_creates_open_tasks_and_leader_assigns_worker() {
    ...
    with_session_test_env(tmp.path(), "swarm-tasks", || {
        let project = tmp.path().join("project");
        let project_str = project.to_string_lossy().to_string();
        let session_id = session_uuid("swarm-tasks-1");
        service::start_session_with_policy(
            "",
            "observer triage",
            &project,
        ...
        let tasks = service::list_tasks(&session_id, None, &project).expect("list tasks");
        ...
        let updated = service::list_tasks(&session_id, None, &project).expect("list tasks");
