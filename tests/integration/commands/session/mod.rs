...
use harness::session::types::{SessionRole, SessionState, SessionStatus, TaskSeverity, TaskStatus};
...
pub(super) fn start_active_session(
    ...
    let _state = service::start_session(context, title, project, Some(session_id)).unwrap();
