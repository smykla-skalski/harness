...
public struct HarnessConnection: Equatable, Sendable {
...
public final class HarnessAPIClient: HarnessClientProtocol {
  private let connection: HarnessConnection
  private let decoder: JSONDecoder
  private let encoder: JSONEncoder
  private let session: URLSession

  public init(connection: HarnessConnection, session: URLSession = .shared) {
  ...
  private func makeRequest(path: String) throws -> URLRequest {
    guard let url = URL(string: path, relativeTo: connection.endpoint) else {
      throw HarnessAPIError.invalidEndpoint(connection.endpoint.absoluteString)
    }
  ...
  private func decodeError(statusCode: Int, data: Data) throws -> HarnessAPIError {
