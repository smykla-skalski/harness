...
public struct DaemonController: DaemonControlling {
  ...
  private let sessionFactory: @Sendable (HarnessConnection) -> any HarnessClientProtocol
  ...
  public init(
    ...
    sessionFactory: @escaping @Sendable (HarnessConnection) -> any HarnessClientProtocol = {
      HarnessAPIClient(connection: $0)
    }
  ) {
  ...
  private func loadManifest() throws -> DaemonManifest {
    let manifestURL = HarnessPaths.manifestURL(using: environment)
  ...
  private func endpointURL(from value: String) throws -> URL {
    guard let url = URL(string: value) else {
      throw HarnessAPIError.invalidEndpoint(value)
    }
    return url
  }
