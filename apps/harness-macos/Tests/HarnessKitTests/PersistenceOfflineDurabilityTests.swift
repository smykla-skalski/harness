...
struct PersistenceOfflineDurabilityTests {
  ...
  init() throws {
    previewContainer = try HarnessModelContainer.preview()
  }
  ...
  func offlineBootstrapRestoresPersistedSelectionAfterRelaunch() async throws {
    ...
    let relaunchedStore = HarnessStore(
  ...
  func liveStoreReopensPersistedSessionsAcrossStoreRecreation() throws {
    ...
    do {
      let firstContainer = try HarnessModelContainer.live(using: environment)
      let firstStore = HarnessStore(
        daemonController: RecordingDaemonController(),
    ...
    let reopenedContainer = try HarnessModelContainer.live(using: environment)
    let reopenedStore = HarnessStore(
      daemonController: RecordingDaemonController(),
