...
actor RecordingDaemonController: DaemonControlling {
  private let client: any HarnessClientProtocol
  private var launchAgentInstalled: Bool
  private var lastEventMessage = "daemon ready"

  init(
    client: any HarnessClientProtocol = PreviewHarnessClient(),
...
final class RecordingHarnessClient: HarnessClientProtocol, @unchecked Sendable {
  enum Call: Equatable {
...
final class FailingHarnessClient: HarnessClientProtocol, @unchecked Sendable {
  private let error: any Error

  init(error: any Error = HarnessAPIError.server(code: 500, message: "internal error")) {
    self.error = error
  }
...
func makeBootstrappedStore(
  client: any HarnessClientProtocol = RecordingHarnessClient()
) async -> HarnessStore {
