...
private func swiftTaskBoardHTTPRoutes() throws -> [TaskBoardHTTPRoute] {
  let relativePath =
    "apps/harness-monitor-macos/Sources/HarnessMonitorKit/API/HarnessMonitorAPIClient+TaskBoard.swift"
  let source = try repoFileContents(relativePath: relativePath)
  let matches = try captures(
    in: source,
...
private func swiftReviewsHTTPRoutes() throws -> [TaskBoardHTTPRoute] {
  let relativePath =
    "apps/harness-monitor-macos/Sources/HarnessMonitorKit/API/HarnessMonitorAPIClient+Reviews.swift"
  let source = try repoFileContents(relativePath: relativePath)
  let matches = try captures(
    in: source,
    pattern: "\\b(get|post|put|delete)\\s*\\(\\s*\"([^\"]+)\""
