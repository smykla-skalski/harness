...
@Suite struct AgentDetailFactZeroHideTests {
  @Test func zeroValueWithFlagHides() {
    let fact = AgentDetailFact(title: "Issues", value: "0", hidesWhenZero: true)
    #expect(fact.isHiddenZero)
  }

  @Test func zeroValueWithoutFlagShows() {
    let fact = AgentDetailFact(title: "Issues", value: "0")
    #expect(!fact.isHiddenZero)
  }

  @Test func nonZeroValueWithFlagShows() {
    let fact = AgentDetailFact(title: "Issues", value: "3", hidesWhenZero: true)
    #expect(!fact.isHiddenZero)
  }

  @Test func paddedZeroStillHides() {
    let fact = AgentDetailFact(title: "Issues", value: "  0  ", hidesWhenZero: true)
    #expect(fact.isHiddenZero)
  }

  @Test func emptyValueDoesNotHide() {
    let fact = AgentDetailFact(title: "Issues", value: "", hidesWhenZero: true)
    #expect(!fact.isHiddenZero)
  }
}

@Suite struct AgentDetailHookPointsGridTests {
  @Test func beforeToolHumanizes() {
  ...
  @Test func beforePromptHumanizes() {
    let hook = HookIntegrationDescriptor(
      name: "BeforePrompt",
      typicalLatencySeconds: 0,
      supportsContextInjection: false
  ...
  @Test func afterPromptHumanizes() {
    let hook = HookIntegrationDescriptor(
      name: "AfterPrompt",
      typicalLatencySeconds: 0,
      supportsContextInjection: true
  ...
  @Test func unknownHookFallsBackToName() {
    let hook = HookIntegrationDescriptor(
      name: "DuringTool",
