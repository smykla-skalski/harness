...
struct SessionWindowCreateFormMetricsTests {
  ...
  func createFormKeepsFocusAndCancelAffordancesInSource() throws {
    ...
    #expect(themeSource.contains("maxWidth: .infinity"))
    #expect(
      themeSource.contains(
        "minHeight: HarnessMonitorNativeTextFieldChromeMetrics.singleLineMinHeight")
    )
    #expect(themeSource.contains("alignment: .leading"))
    #expect(themeSource.contains(".multilineTextAlignment(.leading)"))
    #expect(themeSource.contains("static let cornerRadius: CGFloat = 6"))
    ...
    #expect(themeSource.contains("static let singleLineMinHeight: CGFloat = 28"))
