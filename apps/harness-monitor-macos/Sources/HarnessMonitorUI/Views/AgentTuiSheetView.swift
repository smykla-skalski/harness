...
struct AgentTuiSheetView: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 0) {
      ...
      Divider()
      AgentTuiTabStrip(
        recentSessionIDs: orderedSessionIDs,
        selection: selection,
        titleForSessionID: title(forSessionID:),
        refresh: refresh,
        selectCreateTab: selectCreateTab,
        selectSessionTab: selectSessionTab
      )
      Divider()
      ScrollView {
        paneContent
    ...
    VStack(alignment: .leading, spacing: HarnessMonitorTheme.sectionSpacing) {
      ...
      Text(
        store.selectedAgentTuis.isEmpty
          ...
          : "Open Agent TUI sessions stay pinned in tabs so you can launch another agent without losing the active terminal."
      )
      .scaledFont(.subheadline)
      .foregroundStyle(HarnessMonitorTheme.secondaryInk)
    }
  ...
  private var footer: some View {
    HStack {
      ...
      .accessibilityIdentifier(HarnessMonitorAccessibility.agentTuiCloseButton)
      Spacer()
    }
    .padding(HarnessMonitorTheme.spacingLG)
  }
  ...
  private func selectSessionTab(_ sessionID: String) {
