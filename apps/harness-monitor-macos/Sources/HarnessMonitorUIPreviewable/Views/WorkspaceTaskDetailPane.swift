...
struct WorkspaceTaskDetailPane: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: HarnessMonitorTheme.sectionSpacing) {
      AccessibilityTextMarker(
        identifier: HarnessMonitorAccessibility.workspaceTaskSelection(task.taskId),
        text: task.taskId
      )
      Text(task.title)
      ...
      InspectorSection(title: "Your Notes") {
        if let notesSessionID, isPersistenceAvailable {
          WorkspaceTaskUserNotesSection(
            store: store,
            taskID: task.taskId,
            sessionID: notesSessionID
          )
        } else {
          WorkspaceTaskPersistenceUnavailableState()
        }
      }
    }
    .frame(maxWidth: .infinity, alignment: .leading)
    .accessibilityTestProbe(
      HarnessMonitorAccessibility.workspaceTaskCard,
      label: task.title,
      value: task.taskId
    )
    .accessibilityFrameMarker("\(HarnessMonitorAccessibility.workspaceTaskCard).frame")
  }
}

struct WorkspaceTaskUserNotesSection: View {
  let store: HarnessMonitorStore
  let taskID: String
    ...
    VStack(alignment: .leading, spacing: HarnessMonitorTheme.itemSpacing) {
      ...
      HStack(spacing: HarnessMonitorTheme.itemSpacing) {
        ...
        Button("Add") { submitNote() }
          .harnessActionButtonStyle(variant: .bordered)
          .accessibilityIdentifier(HarnessMonitorAccessibility.workspaceTaskNoteAddButton)
...
struct WorkspaceTaskPersistenceUnavailableState: View {
  var body: some View {
    Text("Persistent notes are unavailable while the local store is offline.")
      .scaledFont(.subheadline)
      .foregroundStyle(HarnessMonitorTheme.secondaryInk)
      .accessibilityIdentifier(HarnessMonitorAccessibility.workspaceTaskNotesUnavailable)
  }
}
