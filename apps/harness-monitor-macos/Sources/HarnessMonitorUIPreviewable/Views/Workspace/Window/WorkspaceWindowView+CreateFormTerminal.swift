    ...
    return AgentsCreateSectionCard {
      VStack(alignment: .leading, spacing: HarnessMonitorTheme.sectionSpacing) {
        ...
        AgentsCreateFieldBlock(
          title: "Display name",
          help: "Required. We pre-fill a suggestion based on the provider."
        ) {
          HStack(spacing: HarnessMonitorTheme.spacingSM) {
            TextField("Display name (required)", text: $formModel.name)
              .harnessNativeTextField()
              .focused(focusedFieldBinding, equals: .name)
              .onChange(of: formModel.name) { _, newValue in
                if newValue != formModel.lastAutoGeneratedName {
                  formModel.lastAutoGeneratedName = nil
                }
              }
              .harnessMCPTextField(
            ...
            .harnessMCPButton(
              HarnessMonitorAccessibility.agentTuiNameSuggestButton,
              label: "Suggest another name",
              pressAction: { regenerateAgentName() }
            )
          }
          if let warning = agentNameValidationWarning {
            Text(warning)
              .scaledFont(.caption)
              .foregroundStyle(HarnessMonitorTheme.caution)
              .accessibilityIdentifier(
                "\(HarnessMonitorAccessibility.agentTuiNameField).warning"
              )
          }
        }
