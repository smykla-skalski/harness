...
public struct ReviewAwaitingBadge: View {
...
public struct ReviewerClaimBadge: View {
  public let taskID: String
...
public struct ReviewQuorumIndicator: View {
  public let taskID: String
...
public struct ReviewPointChip: View {
...
public struct ReviewRoundCounter: View {
  public let taskID: String
...
public struct ReviewImproverCard: View {
...
public struct ReviewStatePanel: View {
  public let task: WorkItem
  ...
  public var body: some View {
    if shouldRender {
      InspectorSection(title: "Review State") {
        VStack(alignment: .leading, spacing: HarnessMonitorTheme.itemSpacing) {
          ReviewFlowRow(spacing: HarnessMonitorTheme.spacingXS) {
            if task.awaitingReview != nil, let awaiting = task.awaitingReview {
              ReviewAwaitingBadge(taskID: task.taskId, awaitingReview: awaiting)
            }
            if task.reviewRound > 0 {
              ReviewRoundCounter(taskID: task.taskId, round: task.reviewRound)
            }
            if task.awaitingReview != nil || task.reviewClaim != nil {
              ReviewQuorumIndicator(
                taskID: task.taskId,
          ...
          if let claim = task.reviewClaim, !claim.reviewers.isEmpty {
            VStack(alignment: .leading, spacing: HarnessMonitorTheme.spacingXS) {
              ForEach(claim.reviewers, id: \.reviewerAgentId) { reviewer in
                ReviewerClaimBadge(taskID: task.taskId, reviewer: reviewer)
              }
            }
          }
