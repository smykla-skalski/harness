...
public struct HarnessMonitorEnvironment: Equatable, Sendable {
  ...
  private static let liveEnvironmentOverrideKeys = [
    ...
    HarnessMonitorRuntimeProfile.launchAgentLabelEnvironmentKey,
...
public enum HarnessMonitorRuntimeProfile {
  ...
  public static let launchAgentLabelEnvironmentKey = "HARNESS_MONITOR_DAEMON_LAUNCH_AGENT_LABEL"
  ...
  static let supportedDerivedDataRoots = Set(["xcode-derived", "xcode-derived-e2e", "xcode-derived-instruments"])
...
public enum HarnessMonitorPaths {
  ...
  public static func launchAgentLabel(
    ...
    if let explicitLabel = normalizedNonEmpty(
      environment.values[HarnessMonitorRuntimeProfile.launchAgentLabelEnvironmentKey]
    ) {
  ...
  private static func resolvedRuntimeProfile(
    using environment: HarnessMonitorEnvironment
  ) -> String? {
  ...
  private static func runtimeProfileBaseRoot(
    using environment: HarnessMonitorEnvironment
  ) -> URL? {
    guard let profile = resolvedRuntimeProfile(using: environment) else {
      return nil
    ...
    let appGroupIdentifier = normalizedAppGroupIdentifier(using: environment)
  ...
  private static func commandEnvironmentEntries(
    using environment: HarnessMonitorEnvironment
  ) -> [(String, String)] {
    ...
    if let commandDataHomeRoot = commandDataHomeRoot(using: environment) {
      entries.append((HarnessMonitorAppGroup.daemonDataHomeEnvironmentKey, commandDataHomeRoot.path))
    }
  ...
  private static func commandDataHomeRoot(
    using environment: HarnessMonitorEnvironment
  ) -> URL? {
    if let configuredRoot = configuredDataHomeRoot(using: environment) {
      return configuredRoot
  ...
  private static func resolvedCodexBridgePortString(
    using environment: HarnessMonitorEnvironment
  ) -> String? {
  ...
  private static func derivedCodexBridgePort(for profile: String) -> Int {
  ...
  private static func inferRuntimeProfile(fromPath rawPath: String?) -> String? {
  ...
  private static func sanitizeRuntimeProfile(_ rawValue: String?) -> String? {
  ...
  private static func shellEscape(_ rawValue: String) -> String {
