...
public final class PreviewHarnessClient: HarnessMonitorClientProtocol, Sendable {
  ...
  func performActionDelay() async throws {
    if let actionDelay {
      try await Task.sleep(for: actionDelay)
    }
  }

  func performReadDelay(for environmentKey: String) async throws {
    guard
  ...
  public func startSession(request: SessionStartRequest) async throws -> SessionStartResult {
    try await performActionDelay()
    return SessionStartResult(sessionId: request.sessionId ?? "sess-preview-new")
  }
