...
public struct SwarmRunLayout {
    ...
    public init(
        ...
        let stateRoot = stateRootOverride
            ?? temporaryDirectory
            .appendingPathComponent("HarnessMonitorSwarmE2E", isDirectory: true)
            .appendingPathComponent(runID, isDirectory: true)
        self.stateRoot = stateRoot
        self.dataRoot = dataRootOverride ?? stateRoot.appendingPathComponent("data-root", isDirectory: true)
        self.dataHome = dataHomeOverride ?? dataRoot.appendingPathComponent("data-home", isDirectory: true)

        let runnerContainerRoot = runnerContainerRootOverride
            ?? syncRootOverride?.deletingLastPathComponent()
            ?? homeDirectory
            .appendingPathComponent("Library/Containers/\(Self.defaultRunnerBundleID)/Data", isDirectory: true)
        self.syncRoot = syncRootOverride
            ?? runnerContainerRoot
        ...
        self.screenRecordingControlDirectory = syncRoot.appendingPathComponent("recording-control", isDirectory: true)
