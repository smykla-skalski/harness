...
public enum BridgeSpawner {
  public enum Failure: Error, CustomStringConvertible {
    ...
    public var description: String {
      ...
      case .exhaustedAttempts(let attempts):
        return "Bridge failed to start after \(attempts) attempts"
      }
    }
  }
  ...
  private static func launch(client: HarnessClient, codexBinary: URL, port: UInt16, logURL: URL)
    throws -> Process
  {
    let process = Process()
  ...
  private static func waitForBridgeReady(
    client: HarnessClient, process: Process, timeout: TimeInterval
  ) -> Bool {
