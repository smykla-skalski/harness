...
private final class SwarmFullFlowRunner {
    ...
    init(assertMode: Bool, environment: [String: String], currentDirectory: URL) throws {
        ...
        let runID = (resolvedRunID?.isEmpty == false ? resolvedRunID : nil)
        ...
        self.destination = environment["HARNESS_MONITOR_XCODEBUILD_DESTINATION"]
            ?? environment["XCODEBUILD_DESTINATION"]
            ?? Self.defaultDestination()
        self.onlyTesting = environment["XCODE_ONLY_TESTING"]
        ...
        self.helperBinary = URL(fileURLWithPath: environment["HARNESS_MONITOR_E2E_TOOL_BINARY"] ?? CommandLine.arguments[0])
    ...
    private func execute() throws {
        ...
        try write(data: SwarmRuntimeProbe.encoded(probeReport), to: layout.stateRoot.appendingPathComponent("probe.json"))
        ...
        let testArgs = [
            ...
            "-scheme", "HarnessMonitorAgentsE2ETests",
    ...
    private func cleanup(status: Int32) -> Int32 {
        ...
        if finalStatus == 0, !keepData {
            fputs("Swarm e2e temp state copied into artifacts and cleaned from: \(layout.stateRoot.path)\n", stderr)
            fputs("Swarm e2e temp sync copied into artifacts and cleaned from: \(layout.syncRoot.path)\n", stderr)
        } else {
    ...
    private func createDirectories() throws {
        ...
        try FileManager.default.createDirectory(at: layout.uiSnapshotsSource, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: layout.screenRecordingControlDirectory, withIntermediateDirectories: true)
    ...
    private func locateGeneratedXctestrun() throws -> URL {
        let products = layout.derivedDataPath.appendingPathComponent("Build/Products", isDirectory: true)
        let contents = try FileManager.default.contentsOfDirectory(at: products, includingPropertiesForKeys: nil)
        ...
        guard let latest = candidates.last else {
            throw CommandFailure(status: 1, message: "Failed to locate generated HarnessMonitorAgentsE2E .xctestrun file")
        }
        return latest
    }
    ...
    private func verifyFinalState() throws {
        ...
        guard result.exitStatus == 0 else {
            throw CommandFailure(status: result.exitStatus, message: "failed to fetch final swarm session status")
        }
        ...
        let hasArbitration = tasks.contains { $0["arbitration"] != nil && !($0["arbitration"] is NSNull) }
        let hasObserveTask = tasks.contains { ($0["source"] as? String) == "observe" }
        guard hasArbitration, hasObserveTask else {
            throw CommandFailure(status: 1, message: "swarm final status missing expected arbitration or observe tasks")
        }
    ...
    private static func resolveCommonRepoRoot(startingAt repoRoot: URL) throws -> URL {
        let process = Process()
        process.executableURL = URL(fileURLWithPath: "/usr/bin/git")
        process.arguments = ["-C", repoRoot.path, "rev-parse", "--path-format=absolute", "--git-common-dir"]
        let stdout = Pipe()
        process.standardOutput = stdout
        process.standardError = Pipe()
        do {
    ...
    private static func resolveHarnessBinary(repoRoot: URL, environment: [String: String]) throws -> URL {
        ...
        process.waitUntilExit()
        let text = String(data: stdout.fileHandleForReading.readDataToEndOfFile(), encoding: .utf8) ?? ""
        guard let line = text.split(separator: "\n").first(where: { $0.hasPrefix("CARGO_TARGET_DIR=") }) else {
...
private final class SwarmActDriverRunner {
    ...
    func run() throws {
        ...
        let leaderID = try joinAgent(role: "leader", runtime: "claude", name: "Swarm Leader", persona: "architect")
        try actReady("act1", values: ["session_id": inputs.sessionID, "leader_id": leaderID])
        try actAck("act1")

        let workerCodexID = try joinAgent(role: "worker", runtime: "codex", name: "Swarm Worker Codex", persona: "test-writer")
        let workerClaudeID = try joinAgent(role: "worker", runtime: "claude", name: "Swarm Worker Claude", persona: "code-reviewer")
        let reviewerClaudeID = try joinAgent(role: "reviewer", runtime: "claude", name: "Swarm Reviewer Claude", persona: "code-reviewer")
        let reviewerCodexID = try joinAgent(role: "reviewer", runtime: "codex", name: "Swarm Reviewer Codex", persona: "code-reviewer")
        let reviewerDuplicateClaudeID = try joinAgent(role: "reviewer", runtime: "claude", name: "Swarm Reviewer Claude Duplicate", persona: "code-reviewer")
        let observerID = try joinAgent(role: "observer", runtime: "claude", name: "Swarm Observer", persona: "debugger")
        let improverID = try joinAgent(role: "improver", runtime: "codex", name: "Swarm Improver", persona: "architect")

        if runtimeAvailable("gemini") {
            _ = try joinAgent(role: "observer", runtime: "gemini", name: "Swarm Observer Gemini", persona: "debugger")
        } else {
            try appendOptionalSkip("gemini")
        }
        if runtimeAvailable("copilot") {
            _ = try joinAgent(role: "improver", runtime: "copilot", name: "Swarm Improver Copilot", persona: "architect")
        } else {
            try appendOptionalSkip("copilot")
        }
        let vibeWorkerID: String
        if runtimeAvailable("vibe") {
            vibeWorkerID = try joinAgent(role: "worker", runtime: "vibe", name: "Swarm Worker Vibe", persona: "generalist")
        } else {
            vibeWorkerID = ""
        ...
        let taskReviewID = try createTask(title: "Review full-flow task", severity: "high", leaderID: leaderID)
        let taskAutospawnID = try createTask(title: "Auto-spawn reviewer task", severity: "medium", leaderID: leaderID)
        let taskArbitrationID = try createTask(title: "Arbitration review task", severity: "high", leaderID: leaderID)
        let taskRefusalID = try createTask(title: "Busy worker refusal task", severity: "low", leaderID: leaderID)
        let taskSignalID = try createTask(title: "Signal collision task", severity: "medium", leaderID: leaderID)
        try actReady("act3", values: [
        ...
        for code in [
            ...
            _ = try SwarmHeuristicInjection.append(.init(
                code: code,
                agentID: observerID,
        ...
        if !currentVibeWorkerID.isEmpty {
            _ = runHarnessMayFail(["session", "leave", inputs.sessionID, currentVibeWorkerID, "--project-dir", inputs.projectDir.path])
            currentVibeWorkerID = try joinAgent(role: "worker", runtime: "vibe", name: "Swarm Worker Vibe Rejoined", persona: "generalist")
        }
        try runHarness(["session", "sync", inputs.sessionID, "--json", "--project-dir", inputs.projectDir.path])
        let temporaryWorkerID = try joinAgent(role: "worker", runtime: "claude", name: "Swarm Temporary Worker", persona: "generalist")
        try runHarness(["session", "leave", inputs.sessionID, temporaryWorkerID, "--project-dir", inputs.projectDir.path])
        ...
        if duplicateClaim.exitStatus == 0 {
            throw Failure(status: 1, message: "duplicate same-runtime review claim unexpectedly succeeded")
        }
        try runHarness([
            "session", "task", "claim-review", inputs.sessionID, taskReviewID,
            "--project-dir", inputs.projectDir.path,
        ...
        try actReady("act9", values: [
            "task_review_id": taskReviewID,
            "reviewer_runtime": "claude",
        ])
        try actAck("act9")

        _ = runHarnessMayFail(["session", "remove", inputs.sessionID, reviewerClaudeID, "--project-dir", inputs.projectDir.path, "--actor", leaderID])
        _ = runHarnessMayFail(["session", "remove", inputs.sessionID, reviewerCodexID, "--project-dir", inputs.projectDir.path, "--actor", leaderID])
        _ = runHarnessMayFail(["session", "remove", inputs.sessionID, reviewerDuplicateClaudeID, "--project-dir", inputs.projectDir.path, "--actor", leaderID])
        try runHarness([
        ...
        try runHarness(["session", "signal", "list", inputs.sessionID, "--json", "--project-dir", inputs.projectDir.path])
        try actReady("act10", values: [
            "task_autospawn_id": taskAutospawnID,
            "worker_claude_id": workerClaudeID,
        ])
        try actAck("act10")
        ...
        try actReady("act11", values: [
            "task_refusal_id": taskRefusalID,
            "worker_claude_id": workerClaudeID,
        ])
        try actAck("act11")

        let reviewerRoundClaudeID = try joinAgent(role: "reviewer", runtime: "claude", name: "Swarm Reviewer Claude Round", persona: "code-reviewer")
        let reviewerRoundCodexID = try joinAgent(role: "reviewer", runtime: "codex", name: "Swarm Reviewer Codex Round", persona: "code-reviewer")
        try assignAndStart(taskID: taskArbitrationID, agentID: workerCodexID, leaderID: leaderID)
        try submitRequestChangesRound(taskID: taskArbitrationID, workerID: workerCodexID, reviewerA: reviewerRoundClaudeID, reviewerB: reviewerRoundCodexID, note: "redoing")
        try actReady("act12", values: [
            "task_arbitration_id": taskArbitrationID,
            "point_id": "p1",
        ])
        try actAck("act12")
        ...
        try submitRequestChangesRound(taskID: taskArbitrationID, workerID: workerCodexID, reviewerA: reviewerRoundClaudeID, reviewerB: reviewerRoundCodexID, note: "round two")
        _ = runHarnessMayFail([
            "session", "task", "update", inputs.sessionID, taskArbitrationID,
            "--project-dir", inputs.projectDir.path,
            "--status", "in_progress",
        ...
        try submitRequestChangesRound(taskID: taskArbitrationID, workerID: workerCodexID, reviewerA: reviewerRoundClaudeID, reviewerB: reviewerRoundCodexID, note: "round three")
    ...
    private func joinAgent(role: String, runtime: String, name: String, persona: String) throws -> String {
    ...
    private func submitRequestChangesRound(
        ...
        let points = #"[{"point_id":"p1","text":"A","state":"open"},{"point_id":"p2","text":"B","state":"open"},{"point_id":"p3","text":"C","state":"open"}]"#
        try runHarness([
    ...
    private func runHarness(_ arguments: [String]) throws -> HarnessClient.Output {
        ...
        guard result.exitStatus == 0 else {
            ...
            throw Failure(status: result.exitStatus, message: "harness \(arguments.joined(separator: " ")) failed: \(stderr)")
        }
        return result
    }
