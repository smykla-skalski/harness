...
public enum PrepareOrchestrator {
    ...
    public static func run(_ inputs: PrepareInputs) throws -> E2EPreparedManifest {
        ...
        try FileManager.default.createDirectory(at: inputs.daemonLog.deletingLastPathComponent(), withIntermediateDirectories: true)
        ...
        guard let codexWorkspace = client.sessionWorkspace(
    ...
    private static func startSession(
        ...
        let result = client.startSession(sessionID: id, title: title, context: context, projectDir: projectDir)
