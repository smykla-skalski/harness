...
struct ResolveCodexLaunch: ParsableCommand {
  static let configuration = CommandConfiguration(
    ...
    abstract:
      "Resolve a supported codex model + effort by inspecting `codex debug models`. Prints '<slug>\\n<effort>' or nothing when no model qualifies."
  )
...
struct ConfigureXctestrun: ParsableCommand {
  static let configuration = CommandConfiguration(
    ...
    abstract:
      "Inject env vars into a generated .xctestrun and write the configured copy. Pass --set KEY=VALUE per env var; blank values are skipped."
  )
  ...
  @Option(
    ...
    help:
      "Repeatable KEY=VALUE pair injected into both env dictionaries; ignored when VALUE is empty.")
  var set: [String] = []
...
struct BridgeReady: ParsableCommand {
  static let configuration = CommandConfiguration(
    ...
    abstract:
      "Read `harness bridge status --json` from stdin; exit 0 when the bridge reports running with required capabilities healthy."
  )
...
struct Prepare: ParsableCommand {
  static let configuration = CommandConfiguration(
    commandName: "prepare",
    abstract:
      "Spawn daemon + bridge, create both Agents e2e sessions, and emit the resulting manifest JSON."
  )
  ...
  @Option(name: .long, help: "Optional fixed codex port; disables retry-on-conflict.")
  var codexPort: UInt16?
  @Option(name: .long, help: "Optional path to write manifest JSON. Defaults to stdout.")
...
struct Teardown: ParsableCommand {
  static let configuration = CommandConfiguration(
    ...
    abstract:
      "Read a prepare manifest, terminate daemon + bridge process trees, and remove state unless --keep-state is set."
  )
...
struct StartRecording: ParsableCommand {
  static let configuration = CommandConfiguration(
    ...
    abstract:
      "Start native screen recording for monitor e2e and keep running until signaled to stop."
  )
  ...
  @Option(
    name: .long,
    help:
      "Optional control dir. When set, wait for start.request, write start.ready once recording begins, and stop after stop.request."
  )
  ...
  @Option(
    name: .long,
    help:
      "Maximum recording duration in seconds before the recorder auto-stops and exits with an error."
  )
...
struct InjectHeuristic: ParsableCommand {
  static let configuration = CommandConfiguration(
    ...
    abstract:
      "Append a deterministic heuristic-trigger fixture to a runtime raw.jsonl. Prints {\"code\": ..., \"log_path\": ...}."
  )
  ...
  var agent: String?
  @Option(
    name: .long, help: "Swarm session ID used to resolve runtime data when --runtime is omitted.")
  ...
  @Option(
    name: .long,
    help: "Optional harness binary path used when runtime lookup requires session status.")
  var harnessBinary: String?

  func run() throws {
    let environment = ProcessInfo.processInfo.environment
    let output = try SwarmHeuristicInjection.append(
      .init(
        code: code,
...
struct SwarmActDriver: ParsableCommand {
  ...
  func run() throws {
    try SwarmFullFlowOrchestrator.runActDriver(
      .init(
