...
struct FrameGapsCommand: ParsableCommand {
    ...
    @Option(
        name: .long,
        parsing: .singleValue,
        help: "Repeatable idle range in seconds (start:end); gaps inside become stalls when over the stall threshold."
    )
...
struct LayoutDriftCommand: ParsableCommand {
    ...
    var threshold: Double = Double(RecordingTriage.layoutDriftPointThreshold)
