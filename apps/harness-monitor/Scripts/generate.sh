...
TUIST_MANAGED_DAEMON_DATA_HOME="$HARNESS_DAEMON_DATA_HOME"
export TUIST_MANAGED_DAEMON_RUNTIME_LANE
TUIST_MANAGED_DAEMON_RUNTIME_LANE="$HARNESS_MONITOR_RUNTIME_LANE"
export TUIST_MANAGED_DAEMON_CODEX_WS_PORT
TUIST_MANAGED_DAEMON_CODEX_WS_PORT="$HARNESS_CODEX_WS_PORT"
...
tuist_env_fingerprint() {
...

  {
    ...
    printf 'TUIST_MANAGED_DAEMON_DATA_HOME=%s\n' \
      "${TUIST_MANAGED_DAEMON_DATA_HOME:-}"
    printf 'TUIST_MANAGED_DAEMON_RUNTIME_LANE=%s\n' \
      "${TUIST_MANAGED_DAEMON_RUNTIME_LANE:-}"
    printf 'TUIST_MANAGED_DAEMON_CODEX_WS_PORT=%s\n' \
      "${TUIST_MANAGED_DAEMON_CODEX_WS_PORT:-}"
...
with open(sys.argv[1]) as f:
    manifest = f.read()
urls = re.findall(r'\.package\(.*?url:\s*"([^"]+)', manifest, re.DOTALL)
