...
class MonitorXcodebuildPolicyTests(unittest.TestCase):
    ...
    def test_mise_monitor_policy_lab_task_uses_the_fixed_user_lane(self) -> None:
        ...
        self.assertIn("export HARNESS_MONITOR_BUILD_LANE", task_body)
        self.assertIn("export HARNESS_MONITOR_RUNTIME_LANE", task_body)
        self.assertIn("export HARNESS_MONITOR_POLICY_LAB_GENERATE", task_body)
        self.assertIn(
    ...
    def test_policy_canvas_lab_capture_uses_the_standalone_lab_host(self) -> None:
        ...
        self.assertIn('HARNESS_MONITOR_POLICY_LAB_READY_FILE=$READY_FILE', script)
        self.assertIn('Policy Canvas Lab did not publish a final route output', script)
        self.assertNotIn('sleep 5', script)
