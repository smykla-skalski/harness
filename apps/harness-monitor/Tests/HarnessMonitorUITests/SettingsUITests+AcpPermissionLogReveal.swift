...
@MainActor
final class SettingsUITestsAcpPermissionLogReveal:
  ...
  WorkspaceWindowUITestSupporting
{
  ...
  private func verifyPermissionLogReveal(
    ...
    selectSettingsSection(
      in: app,
      identifier: Accessibility.settingsDiagnosticsSection,
  ...
  private func scrollDiagnosticsUntilRevealButtonIsVisible(
    ...
    XCTAssertTrue(
      ...
      "Settings window should be open before scrolling Diagnostics"
    )
  ...
  private func diagnosticsDetailScrollTarget(
    ...
    let scrollViewQuery = settingsWindow.descendants(matching: .scrollView)
    ...
    let detailCandidates = candidates.filter { candidate in
      candidate.frame.midX > settingsWindow.frame.midX
    }
