...
final class MobileMacRelaySnapshotSourceTests: XCTestCase {
  func testClientSnapshotSourceMirrorsLiveStateAndCommandPayloads() async throws {
    ...
    let source = makeLiveStateSnapshotSource(now: now)
  ...
  private func makeLiveStateSnapshotSource(now: Date) -> HarnessMonitorClientMobileMirrorSnapshotSource {
    ...
    let acpAgent = makeLiveStateAcpAgent(sessionID: session.sessionId)
    let review = makeLiveStateReview()
    let reviewFiles = makeLiveStateReviewFiles(for: review)
    let reviewTimeline = makeLiveStateReviewTimeline(for: review)
    let taskBoardItem = makeLiveStateTaskBoardItem(sessionID: session.sessionId)
  ...
  private func makeLiveStateAcpAgent(sessionID: String) -> ManagedAgentSnapshot {
    ManagedAgentSnapshot.acp(
      AcpAgentSnapshot(
        acpId: "acp-1",
        sessionId: sessionID,
        agentId: "agent-1",
  ...
  private func makeLiveStateReview() -> ReviewItem {
    ReviewItem(
  ...
  private func makeLiveStateReviewFiles(for review: ReviewItem) -> ReviewsFilesListResponse {
  ...
  private func makeLiveStateReviewTimeline(for review: ReviewItem) -> ReviewsTimelineResponse {
  ...
  private func makeLiveStateTaskBoardItem(sessionID: String) -> TaskBoardItem {
    TaskBoardItem(
      schemaVersion: 1,
      id: "task-1",
