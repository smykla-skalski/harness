...
final class MobileMacRelayExecutorTests: XCTestCase {
  ...
  func testAPIBackedExecutorDispatchesCommandFamilies() async throws {
    ...
    try await dispatchAgentAndTaskBoardFamilies(executor: executor, snapshot: snapshot)
    try await dispatchPullRequestFamilies(executor: executor, snapshot: snapshot)
    try await dispatchRefreshFamilies(executor: executor, snapshot: snapshot)
  ...
  private func dispatchAgentAndTaskBoardFamilies(
  ...
  private func dispatchPullRequestFamilies(
    executor: HarnessMonitorClientMobileRelayCommandExecutor,
    snapshot: MobileMirrorSnapshot
  ) async throws {
    _ = try await executor.execute(
  ...
  private func dispatchRefreshFamilies(
    executor: HarnessMonitorClientMobileRelayCommandExecutor,
    snapshot: MobileMirrorSnapshot
  ) async throws {
    _ = try await executor.execute(
