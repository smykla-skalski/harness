...
struct ApplicationAuditEventTests {
  // Shared with the store-refresh tests in the +StoreRefresh companion, so this
  // is internal rather than private (private would be file-scoped).
  let reviewActionBackfillStorageKey = "dashboard.reviews.recent-actions"
