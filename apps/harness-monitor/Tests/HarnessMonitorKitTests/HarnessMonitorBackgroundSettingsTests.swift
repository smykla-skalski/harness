...
final class BackgroundAssetBundleTests: XCTestCase {
    ...
    let source = try String(
      contentsOf: repoRoot()
        .appendingPathComponent(
          "apps/harness-monitor/Sources/HarnessMonitor/App/HarnessMonitorWindowBackdrop.swift"
        ),
      encoding: .utf8
    )
  ...
  func testThumbnailGenerationSerializesColdDecodes() throws {
    let source = try String(
      contentsOf: repoRoot()
        .appendingPathComponent(
          "apps/harness-monitor/Sources/HarnessMonitorUIPreviewable/Support/"
...
final class BackgroundGalleryPrefetchPlanTests: XCTestCase {
  func testInitialPlanKeepsSelectedBackgroundWarmWithoutPrefetchingWholeLibrary() {
    ...
    let plan = SettingsBackgroundGalleryPrefetchPlan.selections(
      options: options,
  ...
  func testVisibleTileChurnDoesNotChangeGalleryPrefetchPlan() {
    ...
    let expectedStorageValues =
      Array(options.prefix(SettingsBackgroundGalleryPrefetchPlan.initialLimit))
