...
extension HarnessMonitorStoreUpdateStreamTests {
  ...
  func acpRuntimeRestartPurgesStaleStandalonePermissionState() {
    ...
    let staleDecisionPayload = store.acpPermissionDecisionPayload(for: staleDecisionID)
    #expect(staleDecisionPayload?.rawBatch.batchId == "batch-stale")
  ...
  func acpRuntimeClockUsesDaemonHeartbeatWhenAvailable() {
    ...
    let expectedClockNow = Date(timeIntervalSince1970: 220)
    #expect(
      store.currentAcpRuntimeClockNow(at: Date(timeIntervalSince1970: 120))
        == expectedClockNow
    )
  }
