...
struct WebSocketWireTypesDecodingTests {
  ...
  func decodesRequest() throws {
    let json = #"""
      {"id":"req-1","method":"session.detail","params":{"session_id":"s-1"},
      "trace_context":{"traceparent":"abc"}}
      """#
  ...
  func decodesResponseWithError() throws {
    let json = #"""
      {"id":"req-3","error":{"code":"not_found","message":"missing","status_code":404},
  ...
  func decodesPushEvent() throws {
    let json = #"""
      {"event":"session.updated","recorded_at":"2026-06-15T00:00:00Z","session_id":"s-1",
      "payload":{"status":"running"},"seq":42}
      """#
