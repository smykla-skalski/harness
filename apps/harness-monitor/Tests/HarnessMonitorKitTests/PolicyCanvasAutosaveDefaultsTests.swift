...
struct PolicyCanvasAutosaveDefaultsTests {
  @Test("fresh-install default is 10 seconds")
  func defaultIsTenSeconds() {
    #expect(PolicyCanvasAutosaveDefaults.defaultDebounceSeconds == 10)
  }

  @Test("presets are Off and the common windows")
  func presetsCoverOffAndCommonWindows() {
    #expect(PolicyCanvasAutosaveDefaults.presetSeconds == [0, 5, 10, 30, 60])
    #expect(
