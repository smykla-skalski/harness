...
/// src/reviews/files/{mod,blob,viewed,preview}.rs. These
...
/// request/response (line_limit defaulting from files/preview.rs). Mapping
/// these wire types to the rich hand models is a follow-up.
...
struct ReviewsFilesWireTypesDecodingTests {
  ...
  func decodesFileEnums() throws {
    ...
    #expect(
      try decoder.decode(ReviewFileViewedStateWire.self, from: Data("\"dismissed\"".utf8))
        == .dismissed)
    #expect(
      try decoder.decode(ReviewFileServedByWire.self, from: Data("\"github_rest\"".utf8))
        == .githubRest
    )
  ...
  func decodesPreviewRequest() throws {
    ...
    // collected from files/preview.rs.
    let json = #"""
  ...
  func decodesPreviewResponse() throws {
    let json = #"""
      ...
      "status":"modified","additions":1,"deletions":0,"served_by":"github_rest",
    ...
    #expect(response.previews.first?.servedBy == .githubRest)
