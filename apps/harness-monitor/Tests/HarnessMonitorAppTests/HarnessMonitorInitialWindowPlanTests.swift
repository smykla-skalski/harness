...
final class HarnessMonitorInitialWindowPlanTests: XCTestCase {
  ...
  func uiPreviewableSourceFile(named relativePath: String) throws -> String {
  ...
  func seedRestoreSessions(into store: HarnessMonitorStore) throws -> [String] {
    ...
    for summary in summaries {
      XCTAssertTrue(store.sessionIndex.applySessionSummary(summary))
    }
    return summaries.map(\.sessionId)
  }

  func makeRestoredWindow() -> NSWindow {
    NSWindow(
  ...
  func prepareSharedTabbingIdentity(_ window: NSWindow) {
  ...
  func cleanUpWindows(_ windows: [NSWindow]) {
    for window in windows.reversed() {
      window.orderOut(nil)
    }
  }
