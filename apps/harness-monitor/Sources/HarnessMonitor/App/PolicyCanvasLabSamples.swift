...
struct PolicyCanvasLabSample: Identifiable, Equatable, Sendable {
  let id: String
  let name: String
  let document: TaskBoardPolicyPipelineDocument

  static func == (lhs: PolicyCanvasLabSample, rhs: PolicyCanvasLabSample) -> Bool {
    lhs.id == rhs.id
  }
}
...
/// a `Picker` tag.
enum PolicyCanvasLabSelection: Hashable, Sendable {
  case live
  case sample(String)
}

enum PolicyCanvasLabSamples {
  /// Samples ordered simple -> extreme. The lab picker renders this order.
  static let all: [PolicyCanvasLabSample] = [
  ...
  static let defaultSelectionID = "default-like"

  static func sample(id: String) -> PolicyCanvasLabSample? {
    all.first { $0.id == id }
  }
}
