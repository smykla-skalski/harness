...
extension PolicyCanvasViewModel {
  ...
  private func commitSelectedSwitchArmMutation(
    at index: Int,
  ...
  private func commitSelectedSwitchCases(
  ...
  private func selectedSwitchNodeContext() -> SelectedSwitchNodeContext? {
    guard case .node(let id) = selection,
      let node = nodes.first(where: { $0.id == id })
    else {
      return nil
  ...
  private func normalizedSwitchArms(
  ...
  private func switchOutputPortTitles(
    for arms: [TaskBoardPolicySwitchArm]
  ) -> [String] {
    arms.map(\.port) + ["default"]
  }

  private func migratedSwitchEdges(
    for nodeID: String,
  ...
  private func switchCasePortTitle(_ caseNumber: Int) -> String {
    "case_\(caseNumber)"
  }

  private func switchCasePortID(_ caseNumber: Int) -> String {
    "\(PolicyCanvasPortKind.output.rawValue)-\(switchCasePortTitle(caseNumber))"
  }

  private func switchCaseNumber(forPortID portID: String) -> Int? {
...
private struct SelectedSwitchNodeContext {
  let id: String
