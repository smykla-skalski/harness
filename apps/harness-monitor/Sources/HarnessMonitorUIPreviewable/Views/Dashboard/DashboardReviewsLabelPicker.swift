...
func dashboardReviewsLabelSwatchColor(_ hex: String?) -> Color? {
...
/// cap height.
@MainActor
func dashboardReviewsLabelSwatchImage(
  hex: String?,
  diameter: CGFloat = 10,
  canvas: CGFloat = 14
) -> Image {
  let nsColor: NSColor
  if let color = dashboardReviewsLabelSwatchColor(hex) {
...
@MainActor
func dashboardReviewsAvailableLabels(
  ...
  var labelByName: [String: ReviewRepositoryLabel] = [:]
...
/// ordering (already alphabetical from `dashboardReviewsAvailableLabels`).
@MainActor
func dashboardReviewsPartitionFrequent(
  available: [ReviewRepositoryLabel],
  frequentNames: [String]
) -> (frequent: [ReviewRepositoryLabel], rest: [ReviewRepositoryLabel]) {
  guard !frequentNames.isEmpty else { return ([], available) }
  let labelByName = Dictionary(uniqueKeysWithValues: available.map { ($0.name, $0) })
  var frequent: [ReviewRepositoryLabel] = []
  var seen: Set<String> = []
...
@MainActor
struct DashboardReviewsLabelPickerMenu: View {
  let title: String
  ...
  var body: some View {
    Menu(title) {
      DashboardReviewsLabelMenuContent(
        labels: labels,
...
@MainActor
struct DashboardReviewsLabelPickerActionMenu: View {
  let labels: [ReviewRepositoryLabel]
  let frequentNames: [String]
  let showsDescriptions: Bool
  let onSelect: (String) -> Void
...
@MainActor
private struct DashboardReviewsLabelMenuContent: View {
  let labels: [ReviewRepositoryLabel]
  let frequentNames: [String]
  let showsDescriptions: Bool
  let onSelect: (String) -> Void
  ...
  private func labelButton(for label: ReviewRepositoryLabel) -> some View {
    Button {
      onSelect(label.name)
    } label: {
      DashboardReviewsLabelMenuRow(
        label: label,
...
@MainActor
private struct DashboardReviewsLabelMenuRow: View {
  let label: ReviewRepositoryLabel
  ...
  var body: some View {
    Label {
      ...
      dashboardReviewsLabelSwatchImage(hex: label.color)
    }
  }
}
