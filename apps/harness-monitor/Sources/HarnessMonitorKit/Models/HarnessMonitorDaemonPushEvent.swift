...
public struct DaemonPushEvent: Equatable, Identifiable, Sendable {
  ...
  private static func make(from streamEvent: StreamEvent) throws -> Self {
    if let event = try makeGlobalEvent(from: streamEvent) {
      return event
    }
    if let event = try makeDataChangedEvent(from: streamEvent) {
      return event
    }
  ...
  private static func makeGlobalEvent(from streamEvent: StreamEvent) throws -> Self? {
