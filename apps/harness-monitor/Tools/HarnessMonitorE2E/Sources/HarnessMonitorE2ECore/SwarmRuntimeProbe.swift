...
public struct SwarmRuntimeProbe {
  ...
  public func run() -> Report {
    ...
    probeVersionedBinary(
      name: "vibe", required: false, authOK: true, authReason: "available", record: record)
    probeVersionedBinary(
      name: "opencode", required: false, authOK: true, authReason: "available", record: record)
  ...
  private static func defaultCommandRunner(
    ...
    do {
      ...
      return CommandResult(
        exitStatus: 127, stdout: Data(), stderr: Data(String(describing: error).utf8))
    }
