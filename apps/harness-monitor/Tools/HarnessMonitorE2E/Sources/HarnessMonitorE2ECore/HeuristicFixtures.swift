...
public enum HeuristicFixtures {
  ...
  private static func toolResult(id: String, text: String, isError: Bool, toolName: String)
    -> [String: Any]
  {
...
struct AnyCodable: Encodable {
  ...
  private func encodeAny(_ value: Any, to encoder: Encoder) throws {
    ...
    if let dict = value as? [String: Any] {
      ...
      for key in dict.keys.sorted() {
        ...
        try encode(nested, forKey: key, into: &container)
      }
    } else if let array = value as? [Any] {
      var container = encoder.unkeyedContainer()
      for element in array {
        try container.encode(Self(element))
      }
  ...
  private func encode(
    _ value: Any, forKey key: String, into container: inout KeyedEncodingContainer<DynamicKey>
  ) throws {
    let codingKey = DynamicKey(stringValue: key)
    if let nested = value as? [String: Any] {
      try container.encode(Self(nested), forKey: codingKey)
    } else if let array = value as? [Any] {
      try container.encode(Self(array), forKey: codingKey)
    } else if let string = value as? String {
