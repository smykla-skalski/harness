...
final class SwarmSeedStateTests: XCTestCase {
  ...
  func testSeedCreatesHarnessSyncAndLedgerDirectories() throws {
    ...
    XCTAssertTrue(
      FileManager.default.fileExists(atPath: dataHome.appendingPathComponent("harness").path))
    XCTAssertTrue(
      FileManager.default.fileExists(atPath: dataHome.appendingPathComponent("e2e-sync").path))
    XCTAssertTrue(
      FileManager.default.fileExists(atPath: dataHome.appendingPathComponent("e2e-ledger").path))
  }
