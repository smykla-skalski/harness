...
final class SwarmFullFlowContractTests: XCTestCase {
  func testSwarmFullFlowXCUITestIsNotDisabledAtClassSetup() throws {
    let source = try String(
      contentsOf: repoRoot().appendingPathComponent(
        "apps/harness-monitor/Tests/HarnessMonitorAgentsE2ETests/SwarmFullFlowTests.swift"
      ),
      encoding: .utf8
    )
  ...
  func testBuildForTestingSchemeMatchesProjectAndXctestrunContract() throws {
    ...
    let project = try String(
      contentsOf: repoRoot.appendingPathComponent("apps/harness-monitor/Project.swift"),
      encoding: .utf8
    )
  ...
  private func repoRoot() -> URL {
    URL(fileURLWithPath: #filePath)
      ...
      .deletingLastPathComponent()  // harness-monitor
