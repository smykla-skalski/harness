...
<!-- justify: CF-side-effect Hook-enforced runner guards and user approval prompt gates make auto-invocation acceptable here -->
...
- **No autonomous deviations.** user approval prompt before any unplanned change.
...
- **Stop and triage every failure.** On any unexpected result, failure, or mismatch, stop. Classify as suite bug, product bug, harness bug, or environment issue. Present classification to user via a user approval prompt before continuing because unclassified failures corrupt the audit trail. See bug-found gate in Phase 4.
...
1. Set `DATA_DIR` from "Preprocessed context". If missing, ask the user with options:
   - `Create suite with /suite:create`
   - `Provide a different suite path`

2. Resolve `REPO_ROOT`: `--repo` flag > cwd `go.mod` with `kumahq/kuma` > user approval prompt with options:
   - `Provide repo path`
   - `Cancel run`
...
1. **The test groups table is authoritative.** Execute every listed group. Do not mark a group as skip without first calling user approval prompt with options:
...
2. For manifest failures, enter triage (`harness run runner-state --event failure-manifest`), then user approval prompt with first line `suite:run/manifest-fix: how should this failure be handled?`. Include suite target path and error message. Options:
