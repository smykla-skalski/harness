...
fn main() {
    ...
    let out_dir =
        PathBuf::from(env::var_os("OUT_DIR").expect("cargo must set OUT_DIR for build.rs"));
    ...
    write_enforcement_module(&out_dir, &violations)
        .unwrap_or_else(|error| panic!("failed to write file-length enforcement module: {error}"));
}
...
fn write_enforcement_module(out_dir: &Path, violations: &[(String, usize)]) -> io::Result<()> {
    let module_path = out_dir.join("file_length_enforcement.rs");
